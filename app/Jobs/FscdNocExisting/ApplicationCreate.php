...
namespace App\Jobs\FscdNocExisting;
...
class ApplicationCreate extends Job implements SelfHandling,ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($token != '') {
            DB::beginTransaction();
            $FscdNocExistingAppStatus = FscdNocExistingAppStatus::firstOrNew([
                'ref_id' => $this->ref_id,
            ]);

            $postdata =$FscdNocExistingAppStatus->app_create_request;

            $url = config('stakeholder.bfcdc.exiting.service_url') . '/application-create';
            ...
            if($response['http_code'] == 200){
                if(isset($response_json->responseCode) && $response_json->responseCode == '200') {
                    FscdNocExisting::where('id',$FscdNocExistingAppStatus->ref_id)->update(['enoc_tracking_no'=>$response_json->data->tracking_number]);
                    $FscdNocExistingAppStatus->status = 1;
                    $FscdNocExistingAppStatus->completed = 0;
                    $FscdNocExistingAppStatus->request_time = $requestSend;
                    $FscdNocExistingAppStatus->response_time = $responseGet;
                    $FscdNocExistingAppStatus->app_create_response = json_encode($response_json);
                }else{
                    $FscdNocExistingAppStatus->status = -1;
                    $FscdNocExistingAppStatus->completed = 0;
                    $FscdNocExistingAppStatus->request_time = $requestSend;
