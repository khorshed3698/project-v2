...
<script language="javascript">
    ...
    jQuery(function () {
        jQuery('#submitFromPreviewBtn').click(function (e) {
            window.opener.document.getElementById("CdaForm").setAttribute("target", "_self");
            window.opener.jQuery("#CdaForm").submit();
            window.close();
        });
    });
...
//        window.opener.jQuery('#' + id + ' option:[value="' + val + '"]').replaceWith("<option value='" + val + "' selected>" + text + "</option>");
    ...
    jQuery('#CdaForm :input').attr('disabled', true);
...
//        return jQuery(this).find('option:selected').text();
    ...
    function CloseMe()
    {
        ...
        window.opener.jQuery("#CdaForm-p-3").css({"display": "block"});
