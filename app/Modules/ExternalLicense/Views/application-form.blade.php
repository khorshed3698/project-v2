        ...
        <div class="box" id="inputForm">
            <div class="box-body">
                ...
                <div class="panel panel-primary">
                    ...
                    'form-horizontal', 'id' => 'ExternalService',
                    'enctype' =>'multipart/form-data', 'files' => 'true')) !!}
                            ...
                            <div class="hidden">
                                @if(!empty($serviceConfiguration->default_data))
                                    @foreach($serviceConfiguration->default_data as $defaultKey => $default)
                                            <?php
                                            $input_specification_key = isset($serviceConfiguration->input_specification->$defaultKey) ? $serviceConfiguration->input_specification->$defaultKey : '';
                                            $keyTypeClasses = !empty($input_specification_key->additionalClasses) ? $input_specification_key->additionalClasses : '';
                                           
                                            if(is_string($default)){
                                                $fromTable = substr($default, 0, 1);
                                            }
                                            ?>
                                        @if($fromTable == '@' && is_string($default))
                                                <?php
                                                $replacedStringDefault = str_replace('@', '', $default);
                                                ?>
                                            <input type="hidden" value="{{$info->$replacedStringDefault}}"
                                                   class="{{$keyTypeClasses}}" name="{{$defaultKey}}"
                                                   id="{{$defaultKey}}"/>
                                        @elseif($fromTable != '@' && is_string($default))
                                            <input type="hidden" value="{{$default}}" class="{{$keyTypeClasses}}"
                                                   name="{{$defaultKey}}" id="{{$defaultKey}}"/>
                                        @else
                                        @foreach($default as $key => $value)
                                            <input type="hidden" value="{{$value}}" class="{{$keyTypeClasses}}"
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6 col-xs-12">
                                                            ...
                                                            @if($list != '<list>' &&  $sublist != '<sublist>' && $listMultiple != '<listMultiple>' && $sublistMultiple != '<sublistMultiple>')
                                                                ...
                                                                @if ($defaultView == '@')
                                                                        <?php
                                                                        $replacedString = str_replace('@', '', $value);
                                                                        ?>
                                                                    <div class="col-md-7">
                                                                        {!! Form::text("$key",!empty($value)?$info->$replacedString:'',['class' => 'form-control input-md ','id'=>"$key",!empty($value)?'readonly':'']) !!}
                                                                ...
                                                                @else
                                                                    <div class="col-md-7">
                                                                        @if(!empty($input_specification_key->type))
                                                                                ...
                                                                                ?>
                                                                            {!! Form::$keyType("$key",!empty($value)?$value:'',['class' => "form-control input-md ".$keyTypeClasses,'id'=>"$key",!empty($value)?'readonly':'', 'rows' => 2]) !!}
                                                                        @else
                                                                            {!! Form::text("$key",!empty($value)?$value:'',['class' => 'form-control input-md','id'=>"$key",!empty($value)?'readonly':'']) !!}
                                                                        @endif
                                                                    </div>
                                                                @endif
                                                            ...
                                                            @else
                                                                ...
                                                                @if($sublist =='<sublist>')
                                                                        ...
                                                                        ?>
                                                                    <div class="col-md-7">
                                                                        {!! Form::select($key,[], '',['class' => "form-control input-md $keyTypeClasses $key",'id'=>"$key", 'api-url'=>$apiUrl,
                                                                                      'selectedvalue'=>$selectedValue, 'onkeydown'=>"setDependent(this,'".$dependent."')"]) !!}
                                                                    </div>
                                                                        <?php
                                                                        $dependent = '';
                                                                        ?>
                                                                ...
                                                                @if($list =='<list>')
                                                                        ...
                                                                        ?>
                                                                    <div class="col-md-7">
                                                                        {!! Form::select($key,[], '',['class' => "form-control $keyTypeClasses input-md $key",'id'=>"$key",'onkeydown'=>"callListApi(this,'".$apiUrl."','".$selectedValue."')",
                                                                ...
                                                                @if($listMultiple =='<listMultiple>')
                                                                        ...
                                                                        ?>
                                                                    <div class="col-md-7">
                                                                        {!! Form::select($key."[]",[], '',['class' => "form-control input-md select2 $key",'multiple','id'=>"$key",'onkeydown'=>"callListApi(this,'".$apiUrl."','".$selectedValue."')",
                                                                            'onchange'=>"callSublistApi(this,'".$key."')"]) !!}
                                                                    </div>
                                                                @endif
                                                                ...
                                                                @if($sublistMultiple =='<sublistMultiple>')
                                                                        ...
                                                                        ?>
                                                                    <div class="col-md-7">
                                                                        {!! Form::select($key."[]",[], '',['class' => "form-control input-md select2 $key",'id'=>"$key", 'api-url'=>$apiUrl,'multiple',
                                                                                      'selectedvalue'=>$selectedValue, 'onkeydown'=>"setDependent(this,'".$dependent."')"]) !!}
                                                                    </div>
                                                                @endif
                        ...
                        <div class="row">
                            <div class="col-md-6 col-xs-12">
                                <div class="pull-left">
                                    <button type="submit" id="save_as_draft"
                                            class="btn btn-info btn-md cancel"
        ...
        $(document).ready(function () {
            ...
            $('.finish').on('click', function (e) {
                ...
                if (form.valid()) {
                    ...
                    popupWindow = window.open('<?php echo URL::to('licence-application/preview?form=ExternalService@3'); ?>');
                } else {
                    return false;
                }
            });
...
//                        var newInput = $('<label id="label_' + id + '"><br/><b>File: ' + fileNameArr[l - 1] + '</b></label>');
    ...
    function callSublistApi(el, replace) {
        ...
        if (dependent_field != undefined) {
            let subListApiUrl = $("#" + dependent_field).attr('api-url').replace('$.' + el.name, elementID);
    ...
    function callbackResponse(response, [calling_id, selected_value, element_id, element_name, element_calling_id, dependant_select_id]) {
        ...
        if (response.responseCode === 200) {
            $.each(response.data, function (key, row) {
                ...
                if (selected_value == id.split('@')[0] || selected_value == value.toLowerCase()) {
