...
<div class="panel-body">
    <p id="ownerData" style="{{isset($appData->e_tin) ? 'display:none;' : ''}}" class="statictooltip">Please click <a>
            <button id="owner" type="button">here</button>
        </a> to provide Owners/Directors/Head of Entity Information.
    </p>

    <div id="ownerTable" style="{{isset($appData->e_tin) ? '' : 'display:none;'}}">
        <table id="ownerInfo" class="table table-bordered table-hover"
            ...
            @if(isset($appData->e_tin))
                ...
                @foreach($appData->e_tin as $key => $value)
                    ...
                    <tr id="ownerInfoRow{{$inc}}">
                        <td>
                            {!! Form::text('e_tin['.$inc.']',isset($appData->e_tin[$key]) ? $appData->e_tin[$key] :'',['class' => ' form-control required input-md onlyNumber','id'=>"e_tin_$gkey",'placeholder' => '']) !!}
                            {!! $errors->first('e_tin','<span class="help-block">:message</span>') !!}
                        </td>
                        <td>
                            {!! Form::text('owner_name['.$inc.']',isset($appData->owner_name[$key]) ? $appData->owner_name[$key] :'',['class' => ' form-control required input-md','id'=>"owner_name_$gkey"]) !!}
                        </td>
                        <td>
                            {!! Form::select('owner_designation['.$inc.']',[],isset($appData->owner_designation[$key]) ? $appData->owner_designation[$key] :'',['class' => 'form-control required input-md owner_designation','placeholder' => 'Select One','id'=>"owner_designation_$gkey", "owner_designation_$gkey"=>$appData->owner_designation[$key]]) !!}
                            {!! $errors->first('','<span class="help-block">:message</span>') !!}
                        </td>
                        <td>
                            {!! Form::text('share['.$inc.']',isset($appData->share[$key]) ? $appData->share[$key] :'',['class' => ' form-control input-md required onlyNumber share','id'=>"share_$gkey"]) !!}
                        </td>
                        <td>
                            {!! Form::select('identity_category_owner['.$inc.']',[],isset($appData->identity_category_owner[$key]) ? $appData->identity_category_owner[$key] :'',['class' => 'form-control input-md required identity_category','placeholder' => 'Select One', 'id'=>"identity_category_$gkey","identity_category_$gkey"=>$appData->identity_category_owner[$key]]) !!}
                            {!! $errors->first('','<span class="help-block">:message</span>') !!}
                        </td>
                        <td>
                            {!! Form::text('owner_nid['.$inc.']',isset($appData->owner_nid[$key]) ? $appData->owner_nid[$key] :'',['class' => ' form-control input-md owner_nid nid required','style' => 'display:none', 'id'=>"owner_nid_$gkey"]) !!}
                        </td>
                        <td>
                            {!! Form::text('owner_passport_no['.$inc.']',isset($appData->owner_passport_no[$key]) ? $appData->owner_passport_no[$key] :'',['class' => ' form-control input-md owner_passport_no required','style' => 'display:none','placeholder' => '', 'id'=>"owner_passport_no_$gkey"]) !!}
                        </td>
                        <td>
                            {!! Form::select('owner_nationality['.$inc.']',[],isset($appData->owner_nationality[$key]) ? $appData->owner_nationality[$key] :'',['class' => 'form-control input-md owner_nationality required','style' => 'display:none','placeholder' => 'Select One', 'id'=>"owner_nationality_$gkey","owner_nationality_$gkey"=> isset($appData->owner_nationality[$key]) ? $appData->owner_nationality[$key] :'']) !!}
                            {!! $errors->first('','<span class="help-block">:message</span>') !!}

                        </td>
                        <td>
                            {!! Form::text('owner_bin['.$inc.']',isset($appData->owner_bin[$key]) ? $appData->owner_bin[$key] :'',['class' => ' form-control input-md owner_bin required','style' => 'display:none','placeholder' => '', 'id'=>"owner_bin_$gkey"]) !!}
                        </td>
            ...
            @else
                <tr id="ownerInfoRow">
                    <td>
                        {!! Form::text('e_tin[0]','',['class' => ' form-control input-md required','id'=>'e_tin_1','placeholder' => '']) !!}
                        {!! $errors->first('e_tin','<span class="help-block">:message</span>') !!}
                    </td>
                    <td>
                        {!! Form::text('owner_name[0]','',['class' => ' form-control input-md required','id'=>'owner_name_1','placeholder' => '']) !!}
                    </td>
                    <td>
                    ...
                    <td>
                        {!! Form::text('share[0]','',['class' => ' form-control input-md onlyNumber share required','id'=>'share_1','placeholder' => '']) !!}
                    </td>
                    <td>
                    ...
                    <td>
                        {!! Form::text('owner_nid[0]','',['class' => ' form-control input-md owner_nid required','style' => 'display:none', 'id'=>'owner_nid_1']) !!}
                    </td>
                    <td>
                        {!! Form::text('owner_passport_no[0]','',['class' => ' form-control input-md owner_passport_no required','style' => 'display:none','placeholder' => '', 'id'=>'owner_passport_no_1']) !!}
                    </td>
                    <td>
                    ...
                    <td>
                        {!! Form::text('owner_bin[0]','',['class' => ' form-control input-md owner_bin required','style' => 'display:none','placeholder' => '', 'id'=>'owner_bin_1']) !!}
                    </td>
                    <td style="vertical-align: middle; text-align: center">
                        <a class="btn btn-sm btn-primary addTableRows"
                           title="Add more"
    ...
    <p id="ownerCloseData" style="{{isset($appData->e_tin) ? '' : 'display:none;'}}"> Please click <a>
