...
<div class="col-md-6">
    {!! Form::label('','L6. Major Capital Machinery', ['class'=>'col-md-5 required-star']) !!}
    <div class="col-md-7" style="{{isset($appData->physical_condition) ? 'display:none;' : ''}}">
...
<div class="panel-body">
    <div id="MajorCapitalTable" style="{{isset($appData->physical_condition) ? '' : 'display:none;'}}">
        <table id="MajorCapitalInfo" class="table table-bordered table-hover"
            ...
            @if(isset($appData->physical_condition))
                <?php $inc = 0; ?>
                @foreach($appData->physical_condition as $key => $value)
                    <?php
                    $gkey = ($key + 1);
                    ?>
                    <tr id="MajorCapital_row{{$inc}}">
                        <td>
                            {!! Form::text('description['.$inc.']',isset($appData->description[$key]) ? $appData->description[$key] :'',['class' => 'form-control input-md required','id'=>"description_$gkey"]) !!}
                            {!! $errors->first('description','<span class="help-block">:message</span>') !!}
                        </td>
                        <td>
                            {!! Form::hidden('hs_code_major_hidden['.$inc.']',!empty($appData->hs_code_major_hidden[$key]) ? $appData->hs_code_major_hidden[$key] :'',['id'=>"hs_code_major_hidden_$gkey"]) !!}

                            <div class="input-group">
                                {!! Form::text('hs_code_major['.$inc.']',!empty($appData->hs_code_major[$key]) ? $appData->hs_code_major[$key] :'',['class' => 'col-md-7 form-control input-md','placeholder' => '', 'id'=>"hs_code_major_$gkey",'readonly']) !!}
                        ...
                        <td>
                            {!! Form::text('value_bdt['.$inc.']',isset($appData->value_bdt[$key]) ? $appData->value_bdt[$key] :'',['class' => 'form-control input-md required onlyNumber','id'=>"value_bdt_$gkey"]) !!}
                        </td>
                        <td>
                            {!! Form::text('production_capacity['.$inc.']',isset($appData->production_capacity[$key]) ? $appData->production_capacity[$key] :'',['class' => ' form-control input-md required','id'=>"production_capacity_$gkey"]) !!}
                        </td>
                        <td>{!! Form::select('physical_condition['.$inc.']',[],isset($appData->physical_condition[$key]) ? $appData->physical_condition[$key] :'',['class' =>'form-control input-md physical_condition required',
                      'id'=>"physical_condition_$gkey", "physical_condition_$gkey"=>$appData->physical_condition[$key]]) !!}
                        ...
                        <td style="vertical-align: middle; text-align: center">
                            ...
                            <a class="btn btn-sm btn-primary addTableRows"
                               onclick="addTableRowMajorCapital('MajorCapital_body', 'MajorCapital_row{{$inc}}');"><i
                                        class="fa fa-plus"></i></a>
                            <?php } else { ?>
                            {{--                            @if($viewMode != 'on')--}}
                            <a href="javascript:void(0);"
                               class="btn btn-sm btn-danger removeRow"
                               onclick="removeTableRow('MajorCapital_body','MajorCapital_row{{$inc}}');">
    ...
    <p id="MajorCapitalCloseData"
       style="{{isset($appData->physical_condition) ? '' : 'display:none;'}}"> Please click <a>
...
<script>
    ...
    $(document).ready(function () {
        <?php if(!isset($appData->physical_condition)){?>
