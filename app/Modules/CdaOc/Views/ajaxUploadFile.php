...
if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $file_size = round($size / (1024), 1); // megabytes with 1 digit
        ...
        if (in_array($ext, $valid_formats)) {
            if ($file_size <= 300) { // maximum file size 2 MB
                ...
                if (move_uploaded_file($tmp, $path . $ym . $actual_image_name)) {
                    ...
                    echo "<font class=$flag style='font-size: 15px;color:#554d4df0;'>-Uploaded file size is " . number_format($size / 1024) . ' KB </font>';
                } else
                    echo "-failed";
            } else
                echo "Error-File size max 300 KB";
        } else
