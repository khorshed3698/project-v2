...
@section('content')
            ...
            <div class="box">
                <div id="paymentPanel" class="panel panel-default" style="display: none;">
                    <div class="panel-body">
                        {!! Form::open(array('url' => '/cda-oc/payment','method' => 'post','id' => 'NameClearanceForm','role'=>'form','enctype'=>'multipart/form-data')) !!}
    ...
    <script type="text/javascript">
        ...
        function checkOcCDAStatus() {
            var app_id = $('#enc_app_id').val();

            $.ajax({
                url: '/cda-oc/check-payment-info',
                type: "POST",
                data: {
                    enc_app_id: app_id
                },
                ...
                success: function (response) {
                    if (response.responseCode == 1) {
                        if (response.status == -2) {
                            var msg = '';
                            if (response.message.data.resonse && (response.message.data.response.status == 104)) {
                            ...
                            alert(msg);
                            location.replace("/cda-oc/list/" + '{{\App\Libraries\Encryption::encodeId(132)}}');
                        } else if (response.status == -1) {
                            $('#loding-msg-text').text(response.message)
                            myVar = setTimeout(checkOcCDAStatus, 5000);
                            ...
                            alert(response.message);
                            location.replace("/process/cda-oc/view/" + app_id + '/{{\App\Libraries\Encryption::encodeId(132)}}');
                        } else {
                            alert('Whoops there was some problem please contact with system admin.');
                            window.location.reload();
                        }
                    } else {
                        alert('Whoops there was some problem please contact with system admin. responseCode : ' + response.responseCode);
                        window.location.reload();
                    }
        ...
        $(document).ready(function () {
            checkOcCDAStatus();
            countDownTimer('Connecting to CDA Occupancy Server', 'Please wait to connect...', '2:01');
        });
