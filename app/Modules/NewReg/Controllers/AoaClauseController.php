...
//        }
        ...
        try {
            ...
            if (CommonFunction::asciiCharCheck($request->get('clause_title_id'))){
                $appData->clause_title_id = $request->get('clause_title_id');
            }else{
                Session::flash('error', 'non-ASCII Characters found in clause_title_id [AOA-1001]');
                return Redirect::to(URL::previous() . "#step13");
            }
            $nrData = NewReg::find($app_id);
...
//                    $text = $predifined_text.$request->get('clause');
            ...
            elseif ($request->get('clause_title_id') == 13) { // For Quorum board meeting
//                $predifined_text = "$nrData->q_directors_meeting_num ";
...
//            elseif ($request->get('clause_title_id') == 16) {// Qualification share
//                $existing_director_data = AoaInfo::where('clause_title_id', 16 )->where('rjsc_nr_app_id', $app_id)->get();
//                if (count($existing_director_data) > 0){
//                    if (CommonFunction::asciiCharCheck($request->get('clause'))){
//                        $appData->clause = $request->get('clause');
//                    }else{
...
//
//                }else{
//                    $predifined_text = "The Qualification of a Director shall be holding of ($nrData->no_of_qualification_share) (".CommonFunction::convert_number_to_words($nrData->no_of_qualification_share).")share of Tk $nrData->value_of_each_share (".CommonFunction::convert_number_to_words($nrData->value_of_each_share).")each in the capital of the Company in his/her own name alone and not jointly with others ";
////                    $text = $predifined_text.$request->get('clause');
//                    $text = $predifined_text;
//                    if (CommonFunction::asciiCharCheck($text)){
//                        $appData->clause = $text;
