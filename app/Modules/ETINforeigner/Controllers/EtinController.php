        ...
        try {
            ...
            if ($request->get('actionBtn') == 'Submit') {
                ...
                if (count($alreadyPayment) == 0) {
                    ...
                    return $this->spPaymentSubmit($appData->id, $etinRecordNbr);
...
//                ->leftJoin('sec_sub_sector_list', 'sec_sub_sector_list.id', '=', 'apps.business_sub_sector_id')
    ...
    public function spPaymentSubmit($appId, $etinRecordNbr)
    {
        try {
...
//            $paymentInfo->ref_tran_date_time = date('Y-m-d H:i:s'); // need to clarify
            ...
            foreach ($stackholderMappingInfo as $data) {
                ...
                $paymentDetails->payment_config_id = $payment_config->id;
                $paymentDetails->app_id = $appId;
                $paymentDetails->process_type_id = $this->process_type_id;
                $paymentDetails->payment_category_id = $payment_config->payment_category_id;
                $paymentDetails->payment_id = $paymentInfo->id;
                $paymentDetails->payment_date = date('Y-m-d');
                $paymentDetails->ref_tran_no = 'TRN';
                $paymentDetails->receiver_ac_no = $data['receiver_account_no'];
                $paymentDetails->pay_amount = $data['amount'];
                $paymentDetails->contact_name = CommonFunction::getUserFullName();
                $paymentDetails->contact_email = Auth::user()->user_email;
                $paymentDetails->contact_no = Auth::user()->user_phone;
                $paymentDetails->address = Auth::user()->road_no;
            ...
            if ($paymentInsert) {
                ...
                return Redirect::to('spg/initiate/stack-holder/' . $paymentInfo->id);
            }
