<?php
namespace App\Modules\WasaDeepTubewell\Controllers;
...
use App\Modules\WasaDeepTubewell\Models\RequestQueueWasaDt;
use App\Modules\ProcessPath\Models\ProcessList;
use App\Modules\WasaDeepTubewell\Models\WasaDeepTubewell;
...
class WasaDeepTubewellController extends Controller
{
    ...
    {
        $this->process_type_id = 138;
        $this->aclName = 'WasaDeepTubewell';
    }
    ...
    public function appForm(Request $request)
    {
        ...
        if (!$request->ajax())
        {
            return 'Sorry! this is a request without proper way. [DWASADT-1001]';
        }
        ...
        {
            $token = $this->getToken();
            $wasa_service_url = Config('stackholder.WASA_SERVICE_API_URL_DT');
            $viewMode = 'off';
            $mode = '-A-';
...
//            dd($payment_config);
            if (!$payment_config) {
                return response()
                    ->json(['responseCode' => 1, 'html' => "<h4 style='color: #ff0000;margin-top: 250px;text-align: center;'>Payment Configuration Not found [DWASADT-371]</h4>"]);
                return redirect()->to('/dashboard');
            }
            $public_html = strval(view("WasaDeepTubewell::application-form", compact('viewMode', 'mode','payment_config', 'wasa_service_url', 'token')));
    ...
    public function appStore(Request $request)
    {
        ...
        try
        {
            ...
            if ($request->get('app_id'))
            {
                ...
                $appData = WasaDeepTubewell::find($decodedId);
            ...
            return redirect('licence-applications/wasa-dt/list/' . Encryption::encodeId($processData->process_type_id));
    ...
    public function applicationViewEdit($appId, $openMode = '', Request $request)
    {
        ...
        try
        {
            ...
            $appInfo = ProcessList::leftJoin('dwasa_dt_apps as apps', 'apps.id', '=', 'process_list.ref_id')->leftJoin('user_desk', 'user_desk.id', '=', 'process_list.desk_id')
...
//            dd($payment_config);
            ...
            $public_html = strval(view("WasaDeepTubewell::application-form-edit", compact('appInfo', 'appData', 'viewMode', 'mode', 'token', 'appId', 'agent', 'tl_issued_by', 'wasa_service_url','payment_config')));
            return response()->json(['responseCode' => 1, 'html' => $public_html]);
        }
        catch(\Exception $e)
        {
    ...
    public function applicationView($appId, Request $request)
    {
        ...
        $appInfo = ProcessList::leftJoin('dwasa_dt_apps as apps', 'apps.id', '=', 'process_list.ref_id')->leftJoin('user_desk', 'user_desk.id', '=', 'process_list.desk_id')->leftJoin('process_status as ps', function ($join) use ($process_type_id)
        {
        ...
        $public_html = strval(view("WasaDeepTubewell::application-form-view", compact('appInfo', 'appData', 'process_type_id', 'viewMode', 'mode', 'token', 'spPaymentinformation', 'document', 'wasa_service_url', 'agent')));
        return response()->json(['responseCode' => 1, 'html' => $public_html]);
    ...
    {
        return View::make('WasaDeepTubewell::ajaxUploadFile');
    }
    ...
    {
        $wasa_conncection_service_url = Config('stackholder.WASA_SERVICE_API_URL_DT');
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $wasa_conncection_service_url . "/info/doc-list/$connectionType",
        ...
        if ($decoded_response['responseCode'] == 200)
        {
            if ($decoded_response['data'] != '')
            {
                ...
                $html = view("WasaDeepTubewell::dynamic-document", compact('attachment_list', 'clrDocuments', 'app_id'))->render();
            }
        }
        return response()
    ...
    public function submissionJson($app_id, $tracking_no,$statusId)
    {
        ...
        $wasaRequest = RequestQueueWasaDt::firstOrNew(['ref_id' => $app_id]);
        if($statusId == 2){
        ...
        $appData = WasaDeepTubewell::where('id', $app_id)->first();
        ...
        $submissionData['instituteOwner']                   = $masterData->institute_owner_name;
        $submissionData['headOfInstitute']                  = $masterData->head_of_institute;

        $submissionData['connectionAddress1']               = $masterData->install_address;
        $submissionData['connectionAddress2']               = $masterData->install_address;
        ...
        $submissionData['mouza']                  = $masterData->mouza_name;
        $submissionData['sABookNo']                  = $masterData->sa_book_no;
        $submissionData['cSBookNo']                  = $masterData->cs_book_no;
        $submissionData['rSBookNo']                  = $masterData->rs_book_no;


        $submissionData['depth']                            = $masterData->depth;
        $submissionData['powerOfPump']                      = empty($masterData->power_of_pump) ? explode('@', $masterData->power_of_pump) [0] : null;
        $submissionData['screenDiameter']                   = !empty($masterData->screen_diameter) ? explode('@', $masterData->screen_diameter) [0] : null;
        $submissionData['usageType']                        = !empty($masterData->usage_type) ? explode('@', $masterData->usage_type) [0] : null;
        $submissionData['avgDemandWater']                   = $masterData->average_demand;
        $submissionData['dailyPumpRunningTime']             = $masterData->daily_running_time;
        $submissionData['trackingNo']                       = $tracking_no;
    ...
    public function afterPayment($payment_id)
    {
        ...
        try {
            ...
            WasaDeepTubewell::where('id', $processData->ref_id)->update(['is_submit' => 1]);
            $processData->save();

            RequestQueueWasaDt::where('ref_id', $paymentInfo->app_id)->update([
                'status' => '0'
            ]);

            DB::commit();
            Session::flash('success', 'Payment submitted successfully');
            return redirect('licence-applications/wasa-dt/list/' . Encryption::encodeId($this->process_type_id));
        } catch
        (\Exception $e) {
            DB::rollback();
