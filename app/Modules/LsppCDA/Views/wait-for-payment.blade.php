...
@section('content')
            ...
            <div class="box">
                <div id="paymentPanel" class="panel panel-default" style="display: none;">
                    <div class="panel-body">
                        {!! Form::open(array('url' => '/cda-lspp/payment','method' => 'post','id' => 'NameClearanceForm','role'=>'form','enctype'=>'multipart/form-data')) !!}
    ...
    <script type="text/javascript">
        ...
        function checkLSPPStatus() {
            var app_id = $('#enc_app_id').val();

            $.ajax({
                url: '/cda-lspp/check-payment-info',
                type: "POST",
                data: {
                    enc_app_id: app_id
                },
                ...
                success: function (response) {
                    if (response.responseCode == 1) {
                        if (response.status == -2) {
                            ...
                            alert(msg);
                            location.replace("/cda-lspp/list/" + '{{\App\Libraries\Encryption::encodeId(118)}}');
                        } else if (response.status == 0) {
                            myVar = setTimeout(checkLSPPStatus, 5000);
                            ...
                            alert(response.message);
                            location.replace("/process/cda-lspp/view/" + app_id + '/{{\App\Libraries\Encryption::encodeId(116)}}');
                        } else {
