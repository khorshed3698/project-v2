<div style="margin-bottom: 30px;" class="text-center">
    <h3 style="margin-bottom: 5px;">Bangladesh Investment Development Authority (BIDA)</h3>
    <h4 style="margin-top: 0;">{{ trans('messages.authority_text') }}</h4>
    <h4>{{$board_meeting_data->meting_number}}<sup>th</sup> Inter-ministerial meeting agenda</h4>
</div>
...
<div>
    <table class="table table-bordered" aria-label="Detailed Report Data Table">
        ...
        <tr>
            ...
            <td colspan="4">
                <strong>{{$countAllApplication}}</strong>
            </td>
        </tr>
        </tfoot>
    </table>
        ...
        @if($dataRow->agenda_name == 'AGENDA 2' && $dataRow->type == 'A')
            <div style="page-break-after:always;">
                <div class="text-center">
                    <h5>{{$dataRow->agenda_name}}</h5>
                    <h6>{{$dataRow->type}}. {{$dataRow->agenda_heading_title}}</h6>
                </div>
                <table class="table table-bordered" aria-label="Detailed Report Data Table">
                    <thead>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>AC: {{$rowData->authorized_capital}} US$ PC: {{$rowData->paid_up_capital}} US$ </td>
                        {{-- <td>EIE US$ {{$rowData->est_initial_expenses}} EME US$ {{$rowData->est_monthly_expenses}} </td> --}}
                        {{-- <td  @if($ms == 0)style=""@endif > --}}
                        <?php
            ...
            <div style="page-break-after:always;">
                ...
                <table class="table table-bordered" aria-label="Detailed Report Data Table">
                    ...
                    <tr>
                        ...
                        <td>
                            AC: {{$rowData->authorized_capital}} US$ PC: {{$rowData->paid_up_capital}} US$</td>
                        <td>EIE US$ {{$rowData->est_initial_expenses}} EME US$ {{$rowData->est_monthly_expenses}}</td>
                        {{-- <td @if($ms == 0)style=""@endif > --}}
                        <?php
                            // if (!isset($rowData->DocInfo)) {
            ...
            <div style="page-break-after:always;">
                ...
                <table class="table table-bordered" aria-label="Detailed Report Data Table">
                    ...
                    <tr>
                        ...
                        <td>
                            {{$rowData->emp_name}}, {{$rowData->emp_designation}},
                            {{isset($nationality[$rowData->emp_nationality_id])?$nationality[$rowData->emp_nationality_id]:''}}, {{$rowData->emp_passport_no}}
                        </td>
                        <td>
                            {{isset($WP_visaTypes[$rowData->work_permit_type])?$WP_visaTypes[$rowData->work_permit_type] .'Type' :''}}
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if(!empty($rowData->house_local_amount))
                                House rent
                                {{isset($paymentType[$rowData->house_payment_type_id])?'('.$paymentType[$rowData->house_payment_type_id].')' :''}}:
                            ...
                            @if(!empty($rowData->conveyance_local_amount))
                                Conveyance
                                {{isset($paymentType[$rowData->conveyance_payment_type_id])?'('.$paymentType[$rowData->conveyance_payment_type_id].')' :''}}:
                            ...
                            @if(!empty($rowData->ent_local_amount))
                                ...
                                {{isset($paymentType[$rowData->ent_payment_type_id])?'('.$paymentType[$rowData->ent_payment_type_id].')' :''}}:
