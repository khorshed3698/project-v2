...
namespace App\Modules\SbAccountForeign\Controllers;
...
class SbAccountForeignController extends Controller
{
    ...
    {
        $this->process_type_id = 166;
        $this->aclName = 'SbAccountForeign';
    }
    ...
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way. [SBFAO-1001]';
        }

        if (!ACL::getAccsessRight($this->aclName, '-A-')) {
            return response()->json(['responseCode' => 1, 'html' => "<h4 style='color: #ff0000;margin-top: 250px;text-align: center;'>You have no access right! Contact with system admin for more information. [SBFAO-971]</h4>"]);
        }
        try {
            ...
            $public_html = strval(view("SbAccountForeign::application-form", compact('process_type_id', 'viewMode', 'mode', 'document', 'token', 'service_url', 'divisions', 'district', 'payment_config', 'basicAppInfo')));
...
//            } /* End file uploading */
            ...
            if ($request->get('actionBtn') != "draft" && $processData->status_id != 2) {
                ...
                if (!$payment_config) {
                    Session::flash('error', "Payment configuration not found [SBFAO-1123]");
...
//                }
    ...
    public function applicationViewEdit($appId, $openMode = '', Request $request)
    {
        ...
        try {
            ...
            $public_html = strval(view("SbAccountForeign::application-form-edit", compact('appInfo', 'appData', 'process_type_id', 'viewMode', 'mode', 'token', 'service_url', 'payment_config')));
            return response()->json(['responseCode' => 1, 'html' => $public_html]);
        } catch (Exception $e) {
...
//        $dynamic_shortfall = DynamicShortfallAttachmentDPDC::where('ref_id', $appInfo->ref_id)->get();
        ...
        $public_html = strval(view(
            "SbAccountForeign::application-form-view",
    ...
    {
        return View::make('SbAccountForeign::ajaxUploadFile');
    }
...
//            dd($clr_document);
            ...
            $html = view("SbAccountForeign::dynamic-document", compact('attachment_list', 'clrDocuments', 'app_id')
            )->render();
        }
...
//dd($masterData);
            $OrgInfo = [
                ...
                "Addresses" => [
                    ...
                    [
                        "PostOfficeName" => !empty($masterData->present_post) ? $masterData->present_post : '',
                        "MobileNumber" => !empty($masterData->permanent_phone) ? $masterData->permanent_phone : '',
...
//                        "countryCode"=> !empty($masterData->identification_country)?explode('@', $masterData->identification_country)[0] :'',
                ...
                "AccountType" => !empty($masterData->account_type) ? explode('@', $masterData->account_type)[2] : '',
