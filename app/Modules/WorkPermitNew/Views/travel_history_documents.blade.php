<div class="modal fade" id="travelHistoryRecentAttachmentModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content recent_attachment_load_modal"></div>
    </div>
</div>
...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        <div class="table-responsive">
            <table id="attachment_table" class="table table-striped table-bordered display" width="100%" aria-label="Detailed Required attachment">
                <thead>
                <tr>
                    <th>No.</th>
                    ...
                    @foreach($travel_history_document as $row)
                        <tr>
                            ...
                            <td colspan="6">{!! $row->doc_name !!}
                                @if($viewMode != 'on')
                                    <a class='btn btn-xs btn-primary' data-toggle='modal'
                                       data-target='#travelHistoryRecentAttachmentModal'
                                       onclick='openTravelHistoryModal(this)'
                            ...
                            <td colspan="2">
                                ...
                                @if($viewMode != 'on')
                                    <input name="file<?php echo $row->id; ?>"
                                           <?php if (empty($row->doc_file_path) && empty($allRequestVal["file$row->id"]) && $row->doc_priority == "1") {
                                               echo "class='travelFileRequired required'";
                                           } ?>
                                           id="file<?php echo $row->id; ?>" type="file" size="20"
                                           onchange="uploadDocument('preview_<?php echo $row->id; ?>', this.id, 'validate_field_<?php echo $row->id; ?>', '<?php echo $row->doc_priority; ?>', 'travelFileRequired')"/>
                                @endif
                                ...
                                <div id="preview_<?php echo $row->id; ?>" class="reset-attachment">
                                    <input type="hidden" value="<?php echo !empty($row->doc_file_path) ?
                                               $row->doc_file_path : ''?>"
                                           id="validate_field_<?php echo $row->id; ?>"
                                           name="validate_field_<?php echo $row->id; ?>"
                                           class="<?php echo $row->doc_priority == "1" ? "travelFileRequired required" : '';  ?>"/>
                                </div>
                            </td>
                        </tr>
                        <?php $i++; ?>
...
<script>
    ...
    function openTravelHistoryModal(btn) {
        var this_action = btn.getAttribute('data-action');
        var data_target = btn.getAttribute('data-target');
        if (this_action != '') {
            $.get(this_action, function (data, success) {
                if (success === 'success') {
                    $(data_target + ' .recent_attachment_load_modal').html(data);
                } else {
                    $(data_target + ' .recent_attachment_load_modal').html('Unknown Error!');
                }
