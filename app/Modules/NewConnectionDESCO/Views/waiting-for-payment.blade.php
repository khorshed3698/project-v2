...
@section('content')
            ...
            <div class="box">
                <div id="paymentPanel" class="panel panel-default" style="display: none;">
                    <div class="panel-body">
                        {!! Form::open(array('url' => '/new-connection-desco/payment','method' => 'post','id' => 'NameClearanceForm','role'=>'form','enctype'=>'multipart/form-data')) !!}
    ...
    <script type="text/javascript">
        ...
        function checkDESCOStatus() {
            var app_id = $('#enc_app_id').val();

            $.ajax({
                url: '/new-connection-desco/check-payment-info',
                type: "POST",
                data: {
                    enc_app_id: app_id
                },
                ...
                success: function (response) {
                    if (response.responseCode == 1) {
                        if (response.status == -2 || response.status == -3) {
                            ...
                            myVar = setTimeout(checkDESCOStatus, 5000);
                            ...
                            alert(response.message);
                            $('#loding-msg-text').text(response.message)
                            location.replace("/process/new-connection-desco/view/" + app_id + '/{{\App\Libraries\Encryption::encodeId(116)}}');
                        } else {
        ...
        $(document).ready(function () {
            checkDESCOStatus();
            countDownTimer('Connecting to DESCO Server', 'Please wait to connect...', '2:02');
        });
