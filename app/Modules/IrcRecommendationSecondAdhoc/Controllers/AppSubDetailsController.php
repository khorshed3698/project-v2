...
class AppSubDetailsController extends Controller
{
    ...
    public function annualProductionCapacityInfo(Request $request)
    {
        ...
        $annualProduction = SecondAnnualProductionCapacity::select('id')->where('app_id', $app_id)->get();
        foreach ($annualProduction as $value) {
            $rawMeterial = SecondRawMaterial::select('app_id','apc_product_id','percent')->where('app_id', $app_id)->where('apc_product_id', $value->id)->get();
