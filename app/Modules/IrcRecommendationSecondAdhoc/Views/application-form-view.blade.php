                ...
                <div class="pull-right" data-html2canvas-ignore="true">
                    @if(count($inspectionInfo) > 0)
                        @if((Auth::user()->user_type == '5x505' && in_array($appInfo->status_id,[25])) || in_array(Auth::user()->user_type, ['1x101','2x202', '4x404']))
                            ...
                            <a href="{{ url('irc-recommendation-second-adhoc/production-capacity-pdf/'.Encryption::encodeId($appInfo->id)) }}" class="btn btn-sm btn-primary" title="" target="_blank" rel="noopener">Production Capacity</a>
                            @endif
                            {{--                            <a href="{{ url('irc-recommendation-second-adhoc/existing-machines-pdf/'.Encryption::encodeId($appInfo->id)) }}" class="btn btn-sm btn-primary" title="" target="_blank" rel="noopener">Exisiting Machineries</a>--}}
                            @if(!empty($last_inspection_id) && $appInfo->irc_purpose_id != 2)
                                <a href="{{ url('irc-recommendation-second-adhoc/entitlement-paper-pdf/'.Encryption::encodeId($last_inspection_id)) }}" class="btn btn-sm btn-primary" title="" target="_blank" rel="noopener">Entitlement Paper</a>
                            @endif
                        @endif
                    @endif
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7 col-xs-6">
                                    ...
                                    {{-- @if(empty($appInfo->irc_ccie_brows_copy))
                                        <span class="badge badge-warning">Certificate not found</span>
                                    @else
                                        <a href="{{URL::to('/uploads/'.(!empty($appInfo->irc_ccie_brows_copy) ? $appInfo->irc_ccie_brows_copy : ''))}}" target="_blank" rel="noopener" class="btn btn-success btn-sm">View Certificate</a>
                                    @endif --}}
                ...
                <div class="panel panel-info">
                    ...
                    <div class="panel-body">
                        ...
                        <fieldset class="scheduler-border">
                            ...
                            <table aria-label="detailed info" class="table table-striped table-bordered dt-responsive" cellspacing="0"
                                   width="100%">
                                <thead>
                                <tr>
                        ...
                        <fieldset class="scheduler-border">
                            ...
                            @if(in_array($appInfo->irc_purpose_id, [1,3]) && count($annualProductionCapacity) > 0)
                                <fieldset id="annual_raw">
                                    ...
                                    <table aria-label="detailed info" class="table table-striped table-bordered table-responsive" cellspacing="0" width="100%" id="productionCostTbl">
                                        <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>Name of Product</th>
                            ...
                            @if($appInfo->purpose_id != 2 && count($IrcSixMonthsImportRawMaterials) > 0)
                                <fieldset class="scheduler-border">
                                    ...
                                    <table aria-label="detailed info" class="table" width="100%">
                                        ...
                                        <tr>
                                            <td colspan="2" class="text-right v_label">Others :</td>
                                            <td>{{ !empty($appInfo->ins_apc_half_yearly_import_other) ? $appInfo->ins_apc_half_yearly_import_other : "" }}</td>
                                        </tr>
                                        <tr>
