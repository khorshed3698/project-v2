...
trait IRCRegularCertificateService
{
    public static function pdfGenerate($certificateData)
    {
        ...
        $contents = view('CertificateGeneration::ircregular_certificate_pdf', [
            'appInfo' => $data['appInfo'],
            'listOfMechineryImportedSpare'=> $data['listOfMechineryImportedSpare'],
            'director' => $data['director'],
            'director_signature' => $data['director_signature'],
            'ircAdhocType' => $data['ircAdhocType'],
            'qrCode_url' => $qrCode_url,
            'ircAdhocName' =>  $data['ircAdhocName'],
            ...
            <div>
                ...
                <div style="float: left; width: 60.00%;">
                    <div style=" text-align: center">
                        <span style="font-weight: bold"><u>INVESTMENT IS PRIORITY</u></span> <br>
                        ...
                        <span>Registration & Incentives-'.$data['appInfo']->organization_status_name.' Industry</span><br>
                    </div>
                </div>
        ...
        $mpdf->SetSubject("IRC Regular Certificate");
        ...
        <div style="margin-top:20px;">
            <table class="table" width="100%">
                <tr>
                    <td style="width: 75%">
                        <img src="'.$qrCode_url.'" width="70" alt="QR Code" height="70" /><br><br><p style="font-size: 8px; padding-top: 20px;">Please scan for verification.</p><br>
                    </td>
                    ...
                    <img src="'.$data['director_signature'].'" width="70" alt="Director Signature" /><br>
                        ...
                        Registration and Incentive-'.$data['appInfo']->organization_status_name.' Industry <br>
        ...
        $pdfFilePath = getPdfFilePath('IRC-Regular', $certificateData->app_id);
    ...
    {
        $data = [];
        $data['appInfo'] = ProcessList::leftJoin('irc_regular_apps as apps', 'apps.id', '=', 'process_list.ref_id')
            ->leftJoin('irc_common_pool', 'irc_common_pool.regular_adhoc_tracking_no', '=', 'process_list.tracking_no')
            ...
            ->leftJoin('bank as processed_bank', 'processed_bank.id', '=', 'apps.n_bank_id')
            ->leftJoin('irc_regular_purpose', 'irc_regular_purpose.id', '=', 'apps.irc_regular_purpose_id')
            ->leftJoin('irc_regular_inspection', 'irc_regular_inspection.app_id', '=', 'apps.id')
            ...
            ->where('irc_regular_inspection.ins_approved_status', 1)
            ->first([
                'process_list.process_type_id',
                'process_list.ref_id',
                'process_list.tracking_no',
                'process_list.approval_copy_remarks',
                'irc_common_pool.approved_date as app_approved_date',
                'processed_bank.name as n_name',
                'processed_bank_branches.branch_name as n_branch_name',
                'existing_bank.name as bank_name',
                'existing_bank_branches.branch_name as bank_branch_name',
                'apps.*',
                'irc_regular_inspection.*',
                'irc_regular_purpose.*',
            ...
            $data['ircAdhocType'] = DB::table('process_list')
            ->select('process_type_id')
            ->where('tracking_no', $data['appInfo']->irc_ref_app_tracking_no)
            ->first();
        
        if($data['ircAdhocType'] !== null){ // Check if the query result is not null
            $processTypeId = $data['ircAdhocType']->process_type_id;
            if ($processTypeId == 13) {
                $data['ircAdhocName'] = '1st';
            } elseif ($processTypeId == 14) {
                $data['ircAdhocName'] = '2nd';
            } elseif ($processTypeId == 15) {
                $data['ircAdhocName'] = '3rd';
            } else {
                $data['ircAdhocName'] = ''; // Handle other cases if needed
            }
        } else {
            $data['ircAdhocName'] = '';
        }
        ...
        $data['listOfMechineryImportedSpare'] = ListOfMachineryImportedSpareParts::where('app_id', $app_id)
