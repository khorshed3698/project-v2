...
trait PONCertificateService
{
    public static function pdfGenerate($certificateData)
    {
        ...
        $contents = view('CertificateGeneration::pon_certificate_pdf', [
            'appInfo' => $data['appInfo'],
            'director' => $data['director'],
            'director_signature' => $data['director_signature'],
            'paymentData' => $data['paymentData'],
            'metingInformation' => $data['metingInformation'],
        ...
        $mpdf->SetSubject("PON Certificate");
        ...
        $pdfFilePath = getPdfFilePath('PON', $certificateData->app_id);
    ...
    {
        $data = [];
        $data['appInfo'] = ProcessList::leftJoin('pon_apps as apps', 'apps.id', '=', 'process_list.ref_id')
        ...
        // 1st and 2nd Payment Date
        $paymentData  = SonaliPayment::where('process_type_id', $process_type_id)
        ->where('app_id', $app_id)
        ->whereIn('payment_category_id', [1, 2])
        ->orderBy('payment_category_id', 'asc')
        ->get(['payment_date']);
        $data['paymentData'] = $paymentData;

        $metingInformation = CommonFunction::getMeetingInfo($data['appInfo']->process_list_id);
        $data['metingInformation'] = $metingInformation;

        $pnoCompanyOffice = ProjectOfficeNewCompanyOffice::where('app_id', $app_id)
        ->get(['c_company_name'])
        ->toArray();
        $namesArray = array_column($pnoCompanyOffice, 'c_company_name');
        $data['companyNames'] = implode(", ", $namesArray);
