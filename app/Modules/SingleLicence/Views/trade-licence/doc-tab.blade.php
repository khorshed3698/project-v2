                ...
                <ul class="nav nav-tabs" role="tablist">
                    ...
                    @foreach($document as $row)
                        <li role="presentation" class="<?php if ($i == 1) {
                            ...
                            <a href="#tabs{{$i}}" data-toggle="tab">{{$row->doc_name}}</a>
                        </li>
                        <?php $i++; ?>
                    @endforeach
                </ul>
                    ...
                    @foreach($document as $row)
                        <div role="tabpanel" class="tab-pane <?php if ($i == 1) {
                            ...
                            @if(!empty($clrDocuments[$row->id]['file']))
                                ...
                                <?php
                                $fileUrl = public_path() . '/uploads/' . $clrDocuments[$row->id]['file'];

                                if(file_exists($fileUrl)) {
                                ?>
                                <object style="display: block; margin: 0 auto;" width="100%" height="1260"
                                        type="application/pdf"
                                        data="/uploads/<?php echo  $clrDocuments[$row->id]['file'] ?>#toolbar=1&amp;navpanes=0&amp;scrollbar=1&amp;page=1&amp;view=FitH"></object>
                                <?php } else { ?>
