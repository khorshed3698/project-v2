...
use App\Modules\CompanyRegSingleForm\Models\RjscTinVerify;
use Carbon\Carbon;
...
//        dd($request->all());
        ...
        try {
            ...
            $company_type=CompanyRegSingleForm::where('id',$app_id)->first(['entity_type_id']);
...
//            }
            ...
            $usual_residential_district = explode('@', $request->get('usual_residential_district_id'));
            $subscriberData->usual_residential_district_id = !empty($usual_residential_district[0]) ? $usual_residential_district[0] : '';
            $subscriberData->usual_residential_district_name = !empty($usual_residential_district[1]) ? $usual_residential_district[1] : '';
            ...
            $permanent_address_district = explode('@', $request->get('permanent_address_district_id'));
            $subscriberData->permanent_address_district_id = !empty($permanent_address_district[0]) ? $permanent_address_district[0] : '';
            $subscriberData->permanent_address_district_name = !empty($permanent_address_district[1]) ? $permanent_address_district[1] : '';

            $subscriberData->mobile = $request->get('mobile');
            $subscriberData->email = $request->get('email');
            $present_nationality = explode('@', $request->get('present_nationality_id'));
            $subscriberData->present_nationality_id = !empty($present_nationality[0]) ? $present_nationality[0] : '';
            $subscriberData->present_nationality_name = !empty($present_nationality[1]) ? $present_nationality[1] : '';

            $original_nationality = explode('@', $request->get('original_nationality_id'));
            $subscriberData->original_nationality_id = !empty($original_nationality[0]) ? $original_nationality[0] : '';
            $subscriberData->original_nationality_name = !empty($original_nationality[1]) ? $original_nationality[1] : '';
...
//            $subscriberData->dob = $request->get('dob');
            ...
            $subscriberData->is_director = !empty($position[2]) ? $position[2] : '';
...
//            $subscriberData->appointment_date = $request->get('appointment_date');
            ...
            $sequence=CompanyRegSingleForm::find(Encryption::decodeId($request->get('app_id')));
            $sequence->sequence = 4;
    ...
    public function gotowitnessEdit($app_id,$company_type){
        $company_type=Encryption::decodeId($company_type);
        $app_id=Encryption::decodeId($app_id);
        $subscribeData= ListSubscriber::where('app_id', $app_id)
            ->where('is_director',1)
            ->count();
        $subscribeDataall= ListSubscriber::where('app_id', $app_id)
            ->count();
        if($company_type == 1 && $subscribeData < 2){
            Session::flash('error','Need Minimum 2(Two)Director.'.$subscribeData);
            return Redirect::to(URL::previous() . "#step3");
        }else if($company_type == 1 && $subscribeDataall > 50){//company type 1=private
            Session::flash('error','Maximum 50(Fifty) subscriber can add.'.$subscribeDataall);
            return Redirect::to(URL::previous() . "#step3");
        }else if($company_type == 2 && $subscribeData <3){ // company type 2=public
            Session::flash('error','Need Minimum 3(Three)Director.'.$subscribeData);
            return Redirect::to(URL::previous() . "#step3");
        }else if($company_type == 2 && $subscribeDataall <7){
            Session::flash('error','Need minimum 7(seven)subscriber.'.$subscribeDataall);
            return Redirect::to(URL::previous() . "#step3");
        }else{
            Session::put('sequence', 4);
...
//        return json_encode($subscribeData);
    ...
    public function tinResponse(Request $request)
    {
        ...
        if ($rjscData == null) {
            ...
            return response()->json(['responseCode' => 1,  'enc_id' => Encryption::encodeId($rjscData->id), 'status' => -1, 'enc_status' => Encryption::encodeId($status), 'message' => 'Your request in-progress']);
        } elseif ($status == -2) {
