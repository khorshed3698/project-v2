...
class ParticularsController extends Controller
{
    ...
    public function rjscPartiularStore(Request $request)
    {
        ...
        try {
            ...
            $agreementDistrict = explode('@', $request->get('agreement_district_id'));
            $appData->agreement_witness_district_id = !empty($agreementDistrict[0]) ? $agreementDistrict[0] : '';;
            $appData->agreement_witness_district_name = !empty($agreementDistrict[1]) ? $agreementDistrict[1] : '';
            $appData->save();
            $reqData = $request->name_corporation_body;
            RjscNrParticularBody::where('app_id', $app_id)->delete();
            foreach ($reqData as $key => $value) {
                ...
                $particularBody->app_id = $appData->id;
            ...
            if ($request->get('actionBtn') == "draft") {
                ...
                $sequence_no = 2; # If save as draft , data should be current step.
            } else {
