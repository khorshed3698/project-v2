            ...
            <div class="row">
                <div class="col-md-12">
                    <table class="table table-striped table-bordered" width="100%">
                        ...
                        <tr>
                            <th width="15%" rowspan="2" class="text-center">Form name</th>
                            <th width="35%" rowspan="2" class="text-center">Description</th>
                            <th width="40%" colspan="3" class="text-center">Action</th>
                        </tr>
                        <tr>
                            <th width="10%" class="text-center"></th>
                            <th width="22%" class="text-center">Upload</th>
                        ...
                        @foreach($Rjsc_NrForms as $info)
                            @if($info->is_extra == 0)
                                ...
                                <tr>
                                    ...
                                    <td style="">
                                        @if($appInfo->doc_status == 0)
                                            @if(in_array($info->id,$uploadedFile))
                                                <a form_id="{{ Encryption::encodeId($info->id)}}"
                                                   href="/company-registration-sf/get-pdf/{{ Encryption::encodeId($info->id)}}/{{ Encryption::encodeId($applicationId)}}"
...
{{--                    @if(\App\Libraries\CommonFunction::checkFileSubmission($applicationId))--}}
{{--                    <a style="margin-right: 120px; " href="/company-registration-sf/get-pdf/check-all-pdf-files-up/{{ Encryption::encodeId($applicationId)}}" class="pull-right btn btn-success btn-sm documentUrl">--}}
...
<script type="text/javascript">
    ...
    function uploadFilesUpload(base64_img, input, target) {
        ...
        $.ajax({
            url: '{{url('company-registration-sf/new-store/upload')}}',
            type: 'post',
            data: {
                _token: _token,
                app_id: app_id,
    ...
    function uploadDocument(targets, id, vField, isRequired) {
        ...
        // try{
            ...
            var action = "{{url('/company-registration-sf/upload-document')}}";
