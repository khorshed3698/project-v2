            ...
            <div class="panel panel-info">
                ...
                <div class="panel-body">
                    <fieldset>
                        ...
                        {!! Form::open(array('url' => '/company-registration-sf/save-aoa-clause','method' => 'post', 'id' => 'aoaEditForm','enctype'=>'multipart/form-data',
                                ...
                                <div class="form-group">
                                    <div class="row">
                                        <div class="col-md-6 {{$errors->has('clause_title_id') ? 'has-error': ''}}">
                                            ...
                                            <div class="col-md-6">
                                                {!! Form::select('clause_title_id',[],'', ['class' => 'form-control input-md required','placeholder' => 'Select One','required' => 'required' ,'id' => 'clause_title_id']) !!}
                                        ...
                                        @if($totalRec > 0)
                                            @foreach($nrAoaClause as $key => $v_aoa)
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="width: 10px">
                                                        <p>{{$i}}</p>
                                                        @if($totalRec == $key+1 && $appInfo->rjsc_from_submit_status == 0)
                        ...
                        {!! Form::open(array('url' => 'company-registration-sf/company-registration-sf-page/final-submit','method' => 'post', 'id' => 'd','role'=>'form')) !!}
                        @if(ACL::getAccsessRight('NewReg','-E-') && $appInfo->rjsc_from_submit_status == 0)
                        <div class="pull-left">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {!! Form::open(array('url' => '/company-registration-sf/update-aoa-clause','method' => 'put', 'id' => 'aosFormId','enctype'=>'multipart/form-data','files' => true, 'role'=>'form')) !!}
                        <div class="row">
                            <input type="hidden" name="row_id" id="record_id_edit">
                            <div class="col-md-12 {{$errors->has('clause_title_id') ? 'has-error': ''}}">
                                {!! Form::label('clause_title_id','Clause Title', ['class' => 'col-md-3 text-left']) !!}
                                <div class="col-md-9">
                                    {!! Form::select('clause_title_id', [], '', ['class' => 'form-control input-md required','placeholder' => 'Select One','id'=>'clause_title_id']) !!}
                                    {!! $errors->first('clause_title_id','<span class="help-block">:message</span>') !!}
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on('change', '#clause_title_id', function () {
            var title_id = $(this).val().split("@")[0];
            var app_id = $('#app_id').val();


            $.ajax({
                type: "get",
                url: "/company-registration-sf/article-show",
                data: {
        ...
        $('#removeCluaseEdit').click(function () {
            if (confirm('Are you sure to delete')) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo url(); ?>/company-registration-sf/delete-aoa-clause",
                    data: {
                        row_id: row_id,
        ...
        $(function(){
            ...
            $('#clause_title_id').keydown();
        });

        $('#clause_title_id').on('keydown', function(el){
            var key = el.which;
            ...
            var api_url = "{{$rjscBaseApi}}/info/clause-head";
