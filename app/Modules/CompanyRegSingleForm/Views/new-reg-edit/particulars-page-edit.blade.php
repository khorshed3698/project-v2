...
<style>
    ...
    #directorparticulartable tbody tr{
        height:200px;
    }
    #agrementheader{
        margin-top:20px;
                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <div class="col-md-6 pull-right">
                            <a href="/new-reg/new-reg-form-xii-pdf/{{ Encryption::encodeId($appInfo->id)}}"
                        ...
                        <div class="row">
                            <div class="col-md-12 ">
                                <div id="agrementheader"  class="text-center">
                                    <h5 style="font-weight: bold;">PARTICULARS OF THE DIRECTORS,MANAGER AND MANAGING AGENTS AND OF ANY
                                        THEREIN</h5>
                                </div>
                                <div class="text-left">
                                    <p style="width: 450px; display: table;">
                                        <span style="display: table-cell; width: 180px;">Name of the Company:</span>
                                        <span style="display: table-cell; border-bottom: 1px solid black;">{{\App\Libraries\CommonFunction::getCompanyNameById(Auth::user()->company_ids)}}</span>
                                    </p>
                                    <p style="width: 450px; display: table;">
                                        <span style="display: table-cell; width: 180px;">Presented for Filling by:</span>
                                        <span style="display: table-cell; border-bottom: 1px solid black;" id="name"></span>
                                    </p>
                                </div>
                                <div id="directorlist">
                                    {{--<img id="loading-image" src="/assets/images/loadWait.gif" style="display:none;" alt="loadWait"/>--}}
                                    <table id="directorparticulartable" class="table  loading table-bordered">
                                        <thead>
                                        <tr>
                                            <th scope="col" style="text-align: center">The present Names or Names and Surnames</th>
                                            <th scope="col" style="text-align: center">Any former Name or Names or Surnames</th>
                                            <th scope="col" style="text-align: center">Nationality</th>
                                            <th scope="col" style="text-align: center">Nationality of Origin (other than the present Nationality)</th>
                                            <th scope="col" style="text-align: center">Usual Residential Address</th>
                                            <th scope="col" style="text-align: center">Other business, occupation and Directorship, in any, if none, state so (b)</th>
                                            <th scope="col" style="text-align: center">Date of Appointment or Change</th>
                                            <th scope="col" style="text-align: center">Change (c)</th>
                                        </tr>
                                        </thead>
                                        <tbody id="datas">
                                        </tbody>
                                ...
                                <div id="agrementfooter" class="row">
                                    <div class="col-md-6">
                                        <p style="width: 400px; display: table;">
                                            <span style="display: table-cell; width: 100px;">Date of the:</span>
                                    ...
                                    <div id="signature" class="col-md-6 text-center">
                                        <p>(Signature)</p>
                                        <p>Designation</p>
                                        <p>(State whether Director,Managing or Managing Agents)</p>
                                    </div>
                                </div>
                                <p class="text-justify ">
                                    (a) in the ...... of a corporation its corporate name and registered or principal office
                                    should be shown .(b)In the case of as individual who has no business occupation but
                                    any other directorship or directorships...... of the directorship or same of these
                                    Directorship must be entered, Acomplete list of the Directors, Manager, Managing
                                    Agents shown as existing in the last particulars delivered should always be gives.(C)
                                    A note of the change the last list should be made in this columns by placing against a
                                    new directors name the worlds in place of .............. by writing against a new Directors,
                                    name the worlds "deed"........to be as the case may be ,(d) IN case of a firm the full name
                                    address and nationality of each partner and the date .
                            ...
                            <div class="">
                                ...
                                <div class="col-md-6 text-right">
                                    <a href="{{ url('/new-reg-page/memorandum-of-association') }}" class="btn btn-info">Continue</a>
                                </div>
                            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            type: "post",
            url: "/new-reg-page/preview-data-section-12",
            data: {
                _token: _token
            },
            success: function (response) {
                var name = '';
                if(response.RjscWitnessFilledBy){
                    name = response.RjscWitnessFilledBy.name;
                }
                var html = '';
                $.each(response.subscriberList, function (key, row) {
                    html += '<tr>' +
                        '<td>'+row.corporation_body_name+'</td>' +
                        '<td>'+row.former_individual_name+'</td>' +
                        '<td>'+row.nationality+'</td>' +
                        '<td>'+row.present_nationality+'</td>' +
                        '<td>'+row.usual_residential_address+'</td>' +
                        '<td>'+row.directorship_in_other_company+'</td>' +
                ...
                $('#name').html(name);
                $('#datas').html(html);
                $('#directorparticulartable').removeClass('loading');
