            ...
            <div class="box">
                <div id="paymentPanel" class="panel panel-default" style="display: none;">
                    <div class="panel-body">
                        @if($appInfo->expire_date_for_name != '')
                            <h2 class="text-center text-warning">Payment Expire Date : {{$appInfo->expire_date_for_name}} &nbsp; &nbsp; Submission No : {{$appInfo->submission_no}}</h2>
                        @endif

                        <h4>Click the payment button</h4>
                        {!! Form::open(array('url' => '/company-registration-sf/company-registration-sf/payment','method' => 'post','id' => 'NewReg','role'=>'form','enctype'=>'multipart/form-data')) !!}
...
@section('footer-script')
    ...
    <script>
        function callajaxfunction() {
            ...
            $.ajax({
                type: "get",
                url:'/company-registration-sf/check-rjsc-application-status',
                data: {
                    appid: applicationid
                },
                success: function (response) {
                    if (response.responseCode == 1) {
                        if (response.status == 0 || response.status == -1) {
                            ...
                            window.location.replace("{{url('/process/licence-applications/company-registration-sf/view/').\App\Libraries\Encryption::encodeId($app_id).'/'.\App\Libraries\Encryption::encodeId($processTypeId)}}")
                        }
                        else{
                            $('#loading').hide();
        ...
        function checkdocstatus(applicationid) {
            ...
            $.ajax({
                type: "get",
                url:'/company-registration-sf/check-rjsc-doc-status',
                data: {
                    appid: applicationid
                },
                success: function (response) {
        ...
        function paymentstatus(applicationid) {
            ...
            $.ajax({
                type: "get",
                url:'/company-registration-sf/check-rjsc-payment-status',
                data: {
                    appid: applicationid
                },
                success: function (response) {
                    if (response.responseCode == 1) {
                        if (response.status == 0 || response.status == -1) {
                            ...
                            let paymentInformation = response.paymentInformation;
