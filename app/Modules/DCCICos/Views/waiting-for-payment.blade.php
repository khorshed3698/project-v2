<?php
$accessMode = ACL::getAccsessRight('DCCI-COS');
?>
...
@section('content')
            ...
            <div class="box">
                <div id="paymentPanel" class="panel panel-default" style="display: none;">
                    <div class="panel-body">
                        {!! Form::open(array('url' => '/dcci-cos/payment','method' => 'post','role'=>'form','enctype'=>'multipart/form-data')) !!}
    ...
    <script type="text/javascript">
        ...
        function checkDCCIstatus() {
            var app_id = $('#enc_app_id').val();

            $.ajax({
                url: '/dcci-cos/check-payment-info',
                type: "POST",
                data: {
                    enc_app_id: app_id
                },
                ...
                success: function (response) {
                    if (response.responseCode == 1) {
                        ...
                        } else if (response.status == 0 || response.status == 5) {
                            $('#loding-msg-text').text(response.message)
                            setTimeout(checkDCCIstatus, 5000);
                        } else if (response.status == -1) {
                            alert(response.message);
                            location.replace("/process/new-connection-dcci-cos/view/" + app_id + '/{{\App\Libraries\Encryption::encodeId(123)}}');
                        } else if (response.status == 1) {
        ...
        $(document).ready(function () {
            checkDCCIstatus();
            countDownTimer('Connecting to DCCI Server', 'Please wait to connect...', '2:02');
        });
