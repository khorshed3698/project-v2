...
namespace App\Modules\LimaLayoutPlan\Controllers;


use App\Http\Controllers\Controller;

use App\Modules\LabourInspection\Models\DifeRequestQueue;
...
use App\Modules\LimaLayoutPlan\Models\DifeLayoutPlan;
use App\Modules\LimaLayoutPlan\Models\DifeLayoutPlanAttachment;
use App\Modules\LimaLayoutPlan\Models\DifeLayoutPlanApiRequestQueue;
use App\Modules\LimaLayoutPlan\Models\DifeLayoutPlanStatusMapping;


class LimaLayoutPlanController extends Controller
{

    const ACL = 'LimaFactoryLayout';
    ...
    public function __construct()
    {
        ...
        $this->aclName = 'LimaFactoryLayout';
        $this->dife_service_url = Config('stackholder.LIMA_SERVICE_API_URL');
    }

    public function appForm(Request $request)
    {
        if (!$request->ajax()) {
            return 'Sorry! this is a request without proper way. [LimaFactoryLayout-1001]';
        }
        ...
        try {
            ...
            $public_html = strval(view("LimaLayoutPlan::application-form", $data));
...
//                $this->submissionJson($appData->id, $tracking_no, $processData->status_id);
    ...
    public function appFormEdit($appId, Request $request)
    {
        ...
        try {
            ...
            $data['token'] = $this->getToken();
            $data['ml_service_url'] = $this->dife_service_url;
            $public_html = strval(view("LimaLayoutPlan::application-form-edit", $data));
            return response()->json(['responseCode' => 1, 'html' => $public_html]);
        } catch (\Exception $e) {
            dd($e->getMessage(), $e->getLine());
...
//        return  view("RajukLUCGeneral::application-form-view", compact('appInfo', 'appData', 'process_type_id', 'viewMode', 'mode', 'token', 'spPaymentinformation', 'document'));
        ...
        $public_html = strval(view("LimaLayoutPlan::application-form-view", compact('appInfo', 'app_data', 'process_type_id', 'viewMode', 'token', 'spPaymentinformation', 'document')));
        return response()->json(['responseCode' => 1, 'html' => $public_html]);
    ...
    public function submissionJson($app_id, $tracking_no, $statusId)
    {
        ...
        if ($mutationLandRequest->status != 1) {
            ...
            $paramAppdata = [
                ...
                "layout_factory_mail_address" => !empty(Auth::user()->user_email) ? Auth::user()->user_email : '',
                ...
                "created_where" => 'bida_oss',
                "bida_oss_tracking_no" => $tracking_no,

            ];
            $paramAppdata['user'] = [
                'email' => Auth::user()->user_email,
                'mobile' => str_replace('+88','',Auth::user()->user_phone),
                'full_name' => CommonFunction::getUserFullName()
            ];
    ...
    {
        $response = [];
        $ml_service_url = $this->dife_service_url;
        $app_id = $request->app_id;
        $tracking_no = $request->tracking_no;
        $difeLayoutPlan = DifeLayoutPlan::where('id', $app_id)->first();
        if(empty($difeLayoutPlan->dife_trackingt_id)){
            $response['data']['status'] = 'ID not found !';
            return $response;
        }
        $dife_trackingt_id = $difeLayoutPlan->dife_trackingt_id;
        try {
            $token = $this->getToken();
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => $ml_service_url . 'application-status/' . $dife_trackingt_id,
            ...
            if (!empty($decoded_response['responseCode']) && $decoded_response['responseCode'] == 200) {
                $this->updateProcessStatus($app_id, $decoded_response);
            }
            return $decoded_response['data'];
        } catch (\Exception $e) {
            $response = [];
            $response['data']['status'] = $e->getMessage();
            return $response;
        }
    }// end -:- checkStatus()
    ...
    private function updateProcessStatus($app_id, $decoded_response)
    {
        ...
        if(!empty($apiResponseArray['data']['status'])){
            $difeLayoutPlanStatusMapping = DifeLayoutPlanStatusMapping::where('lima_status', $apiResponseArray['data']['status'])->first();
            if(!empty($difeLayoutPlanStatusMapping)){
                $processList  = ProcessList::where('process_type_id', $this->process_type_id)
                    ->where('ref_id', $app_id)
                    ->first();
                $processList->status_id =  $difeLayoutPlanStatusMapping->bida_status_id;
                $processList->save();
            }
        }
...
//                $mlPayment->transaction_amount =  $processData->cda_processing_fee;
    ...
    public function managingAuthorityModal(Request $request)
    {
        ...
        try {
            ...
            $html = view("LimaLayoutPlan::managing-authority.create", compact('modal_token', 'modal_service_url'))->render();
            return response()->json([
                'responseCode' => 1,
                'message' => '',
                'html' => $html
        ...
        try {
            ...
            if ($decoded_response['responseCode'] == 200) {
                if ($decoded_response['data'] != '') {
                    ...
                    $html = view("LimaLayoutPlan::dynamic-document", compact('attachment_list', 'clrDocuments', 'app_id')
                    )->render();
                }
            }
    ...
    {
        return View::make('LimaLayoutPlan::ajaxUploadFile');
    }// end -:- uploadDocument()
