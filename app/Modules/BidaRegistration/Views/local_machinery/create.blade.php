    ...
    <div class="table-responsive">
        <table id="directorTable" class="table table-striped table-bordered dt-responsive" cellspacing="0" width="100%" aria-label="Detailed Report Data Table">
            ...
            <tr id="directorTableRow" data-number="1">
                ...
                <td>
                    {!! Form::text('l_machinery_local_qty[0]', '', ['class' => 'form-control input-md required number']) !!}
                    {!! $errors->first('l_machinery_local_qty','<span class="help-block">:message</span>') !!}
                </td>
                <td>
                    {!! Form::text('l_machinery_local_unit_price[0]', '', ['class' => 'form-control input-md required number']) !!}
                    {!! $errors->first('l_machinery_local_unit_price','<span class="help-block">:message</span>') !!}
                </td>
                <td>
                    {!! Form::text('l_machinery_local_total_value[0]', '', ['class' => 'form-control input-md required number']) !!}
...
<script>
    $(document).ready(function () {
        ...
        function applyValidationRules() {
            $("input[name^='l_machinery_local_qty'], input[name^='l_machinery_local_unit_price'], input[name^='l_machinery_local_total_value']").each(function() {
                $(this).rules("add", {
                    required: true,
                    number: true,
                    min: 0.01
        ...
        if (final_total_row > 3 && check_tfoot_element === 0) {
            ...
            for (var i = 0; i < table_header_columns.length; i++) {
                ...
                if (i === (table_header_columns.length - 1)) {
                    table_footer_th.innerHTML = '<a class="btn btn-sm btn-primary addTableRows" title="Add more" onclick="addTableRowLM(\'' + tableID + '\', \'' + template_row_id + '\')"><i class="fa fa-plus"></i></a>';
                } else {
