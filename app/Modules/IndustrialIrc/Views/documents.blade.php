<div class="table-responsive">
    <h5><strong>(Attach Document (Maximum File Size will be 2 MB. Scan এর সময় DPI 100 এবং Color এ ফাইল Scan করতে হবে।
            ফাইল JPG/PDF ফরম্যাট এ save করতে হবে)</strong></h5>
    <table class="table table-striped table-bordered table-hover ">
        ...
        @foreach($attachment_list as $row)
            ...
            @if(in_array($row['document_id'], [233, 92, 104, 232, 741, 742, 743]) == false)
                ...
                <td colspan="2">
                    ...
                    @if(!empty($clrDocuments[$row['document_id']]))
                        <div class="save_file saved_file_{{$row['document_id']}}">
                            @if($clrDocuments[$row['document_id']]['is_upload'] == 0)
                                <input type="hidden" name="is_uploaded_<?php echo $row['document_id']; ?>" value="0"/>
                                <a target="_blank" class="documentUrl" href="{{!empty($clrDocuments[$row['document_id']]['file']) ?
