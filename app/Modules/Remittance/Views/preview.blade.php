...
<script language="javascript">
    ...
    jQuery(function () {
        jQuery('#submitFromPreviewBtn').click(function (e) {
            window.opener.document.getElementById("RemittanceNewForm").setAttribute("target", "_self");
            window.opener.jQuery("#RemittanceNewForm").submit();
            window.close();
        });
    });
...
//        window.opener.jQuery('#' + id + ' option:[value="' + val + '"]').replaceWith("<option value='" + val + "' selected>" + text + "</option>");
    ...
    window.opener.jQuery(".title ").css({"display": "none"});11
    ...
    jQuery('#RemittanceNewForm :input').attr('disabled', true);
    ...
    jQuery('#previewDiv #accept_terms').attr("onclick", 'return false').attr("checked", "checked");
...
//        return jQuery(this).find('option:selected').text();
    ...
    function CloseMe()
    {
        ...
        window.opener.jQuery("#RemittanceNewForm-p-4").css({"display": "block"});
