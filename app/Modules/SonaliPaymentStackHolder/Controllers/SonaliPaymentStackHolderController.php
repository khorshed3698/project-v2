...
namespace App\Modules\SonaliPaymentStackHolder\Controllers;
...
use App\Modules\SonaliPaymentStackHolder\Models\ApiSonaliPaymentHistory;
...
class SonaliPaymentStackHolderController extends Controller
{
    /**
     * Payment types
     * ================================
    ...
    public function index()
    {
        ...
        try {
            ...
            return view('SonaliPaymentStackHolder::list', compact('ProcessType', 'CompanyInfo', 'PaymentType'));
        } catch (\Exception $e) {
            Session::flash('error', 'Something went wrong! Error: '.$e->getMessage().'[SPS-142]');
 ...
 */
    public function paymentList()
    {
        ...
        $list = SonaliPaymentStackHolders::leftJoin('process_list', function ($join) {
            $join->on('process_list.process_type_id', '=', 'api_stackholder_sp_payment.process_type_id');
            $join->on('process_list.ref_id', '=', 'api_stackholder_sp_payment.app_id');
        })
            ->orderBy('api_stackholder_sp_payment.id', 'desc')
            ->get([
                'api_stackholder_sp_payment.id',
                DB::raw('@rownum  := @rownum  + 1 AS sl_no'),
                'app_tracking_no',
                'contact_email',
                'app_id',
                'api_stackholder_sp_payment.process_type_id',
                'pay_mode_code',
                'transaction_id',
                'request_id',
                'payment_category_id',
...
//                            (in_array($list->payment_category_id, [1, 3]) && ($list->tracking_no == '' or $list->tracking_no == null))
//                            || ($list->payment_category_id == 2))
//                    )
                    if ($list->payment_status == 1 && $list->is_verified == 1) {
                        $btn .= '<a href="' . url('/spg/stack-holder/verifyAndComplete/' . Encryption::encodeId($list->id)) . '" class="btn btn-xs btn-info"><i class="fa fa-check"></i> Verify & Complete</a> <br/>';
                    }
//                    $btn .= '<a href="' . url('/spg/stack-holder/verifyAndComplete/' . Encryption::encodeId($list->id)) . '" class="btn btn-xs btn-info"><i class="fa fa-check"></i> Verify & Complete </a> <br/>';

//                    $btn .= '<a href="' . url('/spg/stack-holder/verify/' . Encryption::encodeId($list->id)) . '" class="btn btn-xs btn-primary"><i class="fa fa-check"></i> Verify</a>';
                    $btn .= '<a href="' . url('/spg/stack-holder/payment-history/' . Encryption::encodeId($list->id)) . '" class="btn btn-xs btn-primary"><i class="fa fa-check"></i> Open</a>';
                    $btn .= ' <a href="'.url('/spg/stack-holder/ref-verification/'.Encryption::encodeId($list->id)).'" class="btn btn-xs btn-info"><i class="fa fa-check"></i> Verify by Ref. No.</a>';
                }
                return $btn;
            })
    ...
    public function indivPaymentHistory($paymentId='')
    {
        ...
        try {
            ...
            return view('SonaliPaymentStackHolder::payment-history', compact('ProcessType', 'CompanyInfo', 'PaymentType', 'paymentId'));
        }
        catch (\Exception $e)
        {
            Session::flash('error', 'Something went wrong! Error: ' . $e->getMessage() . '[SPS-142]');
    ...
    public function indivPaymentHistoryData(Request $request)
    {
        ...
        DB::statement(DB::raw('set @rownum=0'));
        $list = ApiSonaliPaymentHistory::leftJoin('process_list', function ($join) {
            $join->on('process_list.process_type_id', '=', 'api_stackholder_sp_payment_history.process_type_id');
            $join->on('process_list.ref_id', '=', 'api_stackholder_sp_payment_history.app_id');
        })->where('sp_payment_id', $spPaymentId)
            ->orderBy('api_stackholder_sp_payment_history.id', 'desc')
            ->get([
                'api_stackholder_sp_payment_history.id',
                DB::raw('@rownum  := @rownum  + 1 AS sl_no'),
                'app_tracking_no',
                'sp_payment_id',
                'contact_email',
                'app_id',
                'api_stackholder_sp_payment_history.process_type_id',
                'pay_mode_code',
                'transaction_id',
                'request_id',
                'ref_tran_no',
        ...
        return Datatables::of($list)
            ->addColumn('action', function ($list) {
                ...
                if (ACL::getAccsessRight('SonaliPayment', '-A-'))
                {
                    $btn .= '<a href="' . url('/spg/stack-holder/history-verify/' . Encryption::encodeId($list->sp_payment_id)) . '/'. Encryption::encodeId($list->id) . '" class="btn btn-xs btn-primary"><i class="fa fa-check"></i> Verify</a>';
                }
                return $btn;
            })
            ->editColumn('payment_status', function ($list) {
...
//    this function will be uncommented when required
/*
    public function verifyTransactionHistory($id = '', $hist_id = '')
    {
        ...
        try {
            ...
            $spPaymentInfo = SonaliPaymentStackHolders::find($paymentId);
            ...
            if ($verificationStatus && ($spPaymentInfo->is_verified != '1' or $spPaymentInfo->payment_status != '1')) {
                $paymentHistoryInfo = ApiSonaliPaymentHistory::find($historyId);
                ...
                Session::flash('success', 'Payment verification successful. [SPSC-109]');
                return redirect('/spg/stack-holder/');
            }
//
...
//
            ...
            // Session message will be set from transactionHistoryVerification() function
            return redirect()->back();
        } catch (\Exception $e) {
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . "[SPSC-1214]");
            return redirect()->back();
        }
    }
*/
    ...
    public function getPaymentList(Request $request)
    {
        ...
        $list = SonaliPaymentStackHolders::leftJoin('process_list', function ($join) {
            $join->on('process_list.process_type_id', '=', 'api_stackholder_sp_payment.process_type_id');
            $join->on('process_list.ref_id', '=', 'api_stackholder_sp_payment.app_id');
        })->where('api_stackholder_sp_payment.payment_status', 1);
        if (!empty($process_type_id)) {
            $list->where('api_stackholder_sp_payment.process_type_id', $process_type_id);
        }
        if (!empty($company_id)) {
            $list->where('process_list.company_id', $company_id);
        }
        if (!empty($payment_category_id)) {
            $list->where('api_stackholder_sp_payment.payment_category_id', $payment_category_id);
        }
        $list->orderBy('api_stackholder_sp_payment.id', 'desc')
            ->get([
                'api_stackholder_sp_payment.id',
                DB::raw('@rownum  := @rownum  + 1 AS sl_no'),
                'pay_mode',
    ...
    public function transactionHistoryVerification($id)
    {
        try {
            ...
            $spg_config = array(
                ...
                'web_portal_url' => env('web_portal_url'),
                'user_id' => env('user_id'),
                'password' => env('spg_password'),
                'SBL_account1' => env('SBL_account'),
                'SBL_account2' => '',
                'st_code' => env('st_code'),
...
//            'return_url'=>'http://oss-framework.eserve.org.bd/spg/callback'
            ...
            $spg_config = config('payment.spg_settings_stack_holder');
            // dd($spg_config);
            $web_service_url = $spg_config['web_service_url'];
            $user_id = $spg_config['user_id'];
            $password = $spg_config['password'];
            $ownerCode = $spg_config['st_code'];
            $payment_history = ApiSonaliPaymentHistory::find($id);
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: text/xml;charset=utf-8", 'SOAPAction: "http://tempuri.org/TransactionVerification"'));
            $result = curl_exec($ch);
            ...
            Session::flash('error', 'Something went wrong. Error:' . $e->getMessage() . '[SPSM-138]');
            return false;
        }
    }
*/
    ...
    public function transactionVerificationWithRefNo($id)
    {
        ...
        try {
            $web_service_url = config('payment.spg_settings_stack_holder.web_service_url');
            $user_id = config('payment.spg_settings_stack_holder.user_id');
            $password = config('payment.spg_settings_stack_holder.password');
            $ownerCode = config('payment.spg_settings_stack_holder.st_code');

            $paymentInfo = SonaliPaymentStackHolders::find($id);
            if (empty($paymentInfo)) {
                Session::flash('error', 'Invalid payment info [SPS-150]');
                return false;
            }
  ...
  <soap:Body>
    <TransactionVerificationWithRefNo xmlns="http://tempuri.org/">
      ...
      <isEncPwd>true</isEncPwd>
            ...
            if (curl_errno($ch)) {
                Session::flash('error', 'Request Error: ' . curl_error($ch) . '[SPS-151]');
                return false;
            }
            $xml_parser = xml_parser_create();
            xml_parse_into_struct($xml_parser, $result, $val);
            ...
            $paymentInfo->save();
            Session::flash('error', 'Invalid verification response [SPS-152]');
            ...
            Session::flash('error', 'Something went wrong. Error:' . $e->getMessage() . '[SPS-153]');
            return false;
        }
    }
//
...
//     * Store Payment info
//     */
//    public static function AppSubmissionPayment($process_type_id, $app_id){
//        try{
//
...
//            if(!$payment_config){
//                dd('Payment configuration not found [SPC-100]');
//            }
//            // Get SBL payment configuration
//            $spg_config =  $spg_config = array(
...
//            $paymentInfo->app_id = $app_id;
//            $paymentInfo->process_type_id = $process_type_id;
...
////            $paymentInfo->pay_amount = 200;
//            $paymentInfo->contact_name = \App\Libraries\CommonFunction::getUserFullName();
//            $paymentInfo->contact_no = Auth::user()->user_phone;
//            $paymentInfo->address = Auth::user()->road_no .  (!empty(Auth::user()->house_no) ? ', ' . Auth::user()->house_no : '');
//            $paymentInfo->sl_no = 1; // Always 1
//            $paymentInfo->save();
//
//            // if you want to invoke a static method, you should use the self::
//            // and define your method as static
//            //self::initiatePayment($paymentInfo->id);
//            return redirect('spg/initiate/'.$paymentInfo->id);
//        }catch (\Exception $e){
//            dd('Submission payment error [SPC-101]');
//        }
//    }
...
//        if (env('payment_mode', 'off') == 'off'){
//            $ref_url = $_SERVER["HTTP_REFERER"];
//            $url_sec = explode('/', $ref_url);
//            $process_info = ProcessType::where('id', Encryption::decodeId($url_sec[7]))->first();
//            $process_info = ProcessType::where('id', 107)->first();
//            $process_list = ProcessList::
//            where('process_type_id', Encryption::decodeId($url_sec[7]))
//                ->where('ref_id',Encryption::decodeId($url_sec[6]))->first();
//
//            $sonaliPayment = SonaliPaymentStackHolders::where('id', $paymentId)->first();
        ...
        if($paymentId != null){
            try {
                $spg_config = array(
                    ...
                    'user_id' => env('spg_user_id'),
                    'password' => env('spg_password'),
                    'SBL_account1' => env('spg_SBL_account'),
                    'SBL_account2' => '',
...
//            'return_url'=>'http://oss-framework.eserve.org.bd/spg/callback'
                ...
                $sblAccount1 = $spg_config['SBL_account1'];
                $sblAccount2 = $spg_config['SBL_account2'];

                $paymentInfo = SonaliPaymentStackHolders::find($paymentId);


                $account_details = StackholderSonaliPaymentDetails::where('payment_id', $paymentInfo->id)->get();


//                dd($paymentInfo->pay_amount);
                if(!$paymentInfo){
                    dd('Payment record not found ! [SPC-102]');
                }
                $body = '<?xml version="1.0" encoding="utf-8"?>
                 <soap:Envelope 
                ...
                curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
                curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: text/xml;charset=utf-8", 'SOAPAction: "http://tempuri.org/GetSessionKey"'));
                $result = curl_exec($ch);
                if (curl_errno($ch)) {
                ...
                if (!isset($sessTokenDecode)) {
                    dd("Session token not found. [SPSC-103]");
                }
                $sessionToken = $sessTokenDecode->scretKey;


                // payment request info save in JSON
                $paymentReqInfo['ApiAccessUserId'] = $spg_config['user_id'];
                ...
                $paymentReqInfo['ReturnUrl'] = $spg_config['return_url'];
                $paymentReqInfo['TranAmount'] = $paymentInfo->pay_amount;
//                $paymentReqInfo['TranAmount'] = 200;
                $paymentReqInfo['VatPercent'] = $paymentInfo->vat_on_transaction_charge;
...
<Address>".$paymentInfo->address."</Address>
 ...
 <CreditInformation>";
                ...
                foreach ($account_details as $data){
                    $account_no = $data->receiver_ac_no;
                    $amount_no = $data->pay_amount;

                    $body .="<CreditInfo>
<SLNO>$sl</SLNO>
<CreditAccount>".$account_no."</CreditAccount>
<CrAmount>$amount_no</CrAmount>
...
<Onbehalf>$paymentInfo->contact_name</Onbehalf>
                ...
                return view('SonaliPaymentStackHolder::paymentRequest', compact('sessionToken', 'spg_config', 'paymentInfo','account_details'));

            } catch (\Exception $e) {
                dd($e->getMessage() . $e->getFile() . $e->getLine() . '[SPC-105]');
            }
        }
        else{
            dd('Invalid request! [SPSC-104]');
            return redirect()->back();
        }
    }
    ...
    public function callback(Request $request)
    {
        ...
        try {
            ...
            $payment_mode = 'online';
            if ($pay_mode_code == 'A01') { // TODO:: Needed to create CRON scheduler
                $payment_mode = 'offline'; //counter based payment
            }

            DB::beginTransaction();

            $sonaliPayment = SonaliPaymentStackHolders::where('ref_tran_no', $refTranNo)
...
//            dd($sonaliPayment);
            ...
            $sonaliPayment->payment_status = ($transaction_status == 200) ? 1 : (($pay_mode_code == 'A01') ? -1 : -2);
...
//            dd($sonaliPayment);
            ...
            }else if ($transaction_status == '5017' && $pay_mode_code == 'A01') {
...
//                }
            ...
            /// redirect to corresponding application list with payment failure mesassage
            return redirect($process_info->form_url.'/list/'.Encryption::encodeId($process_info->id))->with('error', 'Payment submission failed');
        } catch (\Exception $e) {
            DB::rollback();
            // dd($e->getMessage() . $e->getFile() . $e->getLine() . '[SPC-107]');
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage())."[SPSC-107]");
            return redirect()->back();
        }
    }
...
//       }
        ...
        try {
            ...
            if ($paymentId != null) {
                ...
                $user_id = config('payment.spg_settings_stack_holder.user_id');
                $password = config('payment.spg_settings_stack_holder.password');
//                $sblAccount = config('payment.spg_settings_stack_holder.SBL_account');
                $payment_callback_url = config('payment.spg_settings_stack_holder.return_url_m');

                $decodePaymentId = Encryption::decodeId($paymentId);
                $paymentInfo = SonaliPaymentStackHolders::find($decodePaymentId);
                $payment_details = StackholderSonaliPaymentDetails::where('payment_id', $paymentInfo->id)->get();
                ...
                if (empty($paymentInfo) || $payment_details->isEmpty()) {
                    DB::rollback();
                    Session::flash('error', 'Payment record not found! [SPSM-102]');
                    return redirect()->back();
                }
...
//                $paymentInfo->ref_tran_no = $ref_tran_no;
                ...
                if (!empty($paymentInfo->request_id)) {
                    ...
                    if ($paymentVerifyCheck['code'] == '201') {
                        ...
                        if ($time_diff_minute < $waiting_minutes) {
                            DB::commit();
                            Session::flash('error', 'Please try again after ' . (($waiting_minutes + 1) - $time_diff_minute) . ' minutes or call to help line. [SPSM-107]');
                            return redirect()->back();
                        }
                    }
                }
                ...
                $send_request_id = config('payment.spg_settings_stack_holder.request_id_prefix') . rand(1000000, 9999999); // Will be change later
                ...
                if (curl_errno($ch)) {
                    DB::rollback();
                    Session::flash('error', 'Request Error: ' . curl_error($ch) . '[SPSM-139]');
                    return redirect()->back();
                }
...
<head></head>
<body onload='document.forms[0].submit()'>
<form name='PostForm' method='POST' action=" . config('payment.spg_settings_stack_holder.web_portal_url') . ">
<textarea name='datarequest' id='datarequest' rows='15' style='width:100%; display:none;'>
<SpsRequestByeChallan>
<RequestInformation>
 <Authentication>
...
<Address>" . $paymentInfo->address . "</Address>
  ...
  <CreditInformation>" . $creditInfo . "</CreditInformation>
                ...
                DB::commit();
                return view('SonaliPaymentStackHolder::paymentRequestMultiple',
                    compact('sessionToken','paymentInfo', 'payment_details'));
            } else {
                DB::rollback();
                Session::flash('error', 'Invalid request! [SPSM-104]');
                return redirect()->back();
            }
        } catch (\Exception $e) {
            DB::rollback();
            Session::flash('error', 'Something went wrong! . Error: ' . $e->getMessage() . '[SPSM-105]');
            return redirect()->back();
        }
    }
    ...
    public function callbackMultiple(Request $request)
    {
        try {
            ...
            if ($xml === false) {
                ...
                Please contact with Support team with your application tracking number. [SPSM-146]');
                return redirect('dashboard');
            }
...
//            $sonaliPayment->payment_status = ($transaction_status == 200) ? 1 : (($pay_mode_code == 'A01') ? 3 : -2);
            ...
            */
            if ($transaction_status == '5555') {
                Session::flash('error', 'Payment cancelled by user. [SPSM-147]');
                return redirect('dashboard');
            }
            ...
            if ($transaction_status == '200') {
                ...
                return redirect($process_info->form_url . '/list/' . Encryption::encodeId($process_info->id))
                    ...
                    with this Transaction ID : ' . $transaction_id . '. [SPSM-134]');
            } else {
                /*
                 * if transaction status is 5017 and payment mode is 'A01'
                 * then go to afterCounterPayment function to corresponding module
            ...
            DB::rollback();
            Session::flash('error',
                'Payment callback error. Message : ' . $e->getMessage() . '[SPS-132]');
            return redirect('dashboard');
        }
    }
   ...
   */
    ...
    {
        try {
            $web_service_url = config('payment.spg_settings_stack_holder.single_details_url');
            $user_id = config('payment.spg_settings_stack_holder.user_id');
            $password = config('payment.spg_settings_stack_holder.password');
            $ownerCode = config('payment.spg_settings_stack_holder.st_code');
            ...
            $paymentInfo = SonaliPaymentStackHolders::find($id);
            if (empty($paymentInfo)) {
                Session::flash('error', 'Invalid payment info [SPDV-1326]');
                return false;
            }
            ...
            $processTypeKey = ProcessType::where('id', $paymentInfo->process_type_id)->value('type_key');

            foreach ($result_json as $key => $value) {

                if($processTypeKey=='external-service'){
                    $paymentDetailsInfo = StackholderSonaliPaymentDetails::where([
                        'payment_id' => $id,
                        'receiver_ac_no' => $value->TranAccount,
                        'sl_no' => $value->SlNo
                    ])->first();
                }else{
                    $paymentDetailsInfo = StackholderSonaliPaymentDetails::where(['payment_id' => $id, 'receiver_ac_no' => $value->TranAccount])->first();
                }
                ...
                $paymentDetailsInfoJson['SlNo'] = $value->SlNo;
                $paymentDetailsInfoJson['SCRL_NO'] = !empty($value->SCRL_NO) ? $value->SCRL_NO : null;
                $paymentDetailsInfoJson['ONBEHALF'] = $value->ONBEHALF;
...
//            else{
//                Session::flash('error', 'Payment verification failed. [SPC-1081]');
//            }
            DB::commit();
            return redirect('/spg/stack-holder');
        } catch (\Exception $e) {
    ...
    public function verifyAndComplete($id)
    {
        ...
        try {
            ...
            $paymentId = Encryption::decodeId($id);
            // get payment info
            $paymentInfo = SonaliPaymentStackHolders::where('id', $paymentId)->first();
...
//            $verificationStatus = $this->transactionVerification($paymentId);
            ...
            DB::rollback();
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage())."[SPSC-1215]");
            return redirect()->back();
        }
    }
    ...
    public function transactionVerification($id)
    {
        try {
            ...
            if (empty($paymentInfo)) {
                Session::flash('error', 'Invalid payment info [SPSM-136]');
                return false;
            }
            ...
            Session::flash('error', 'Invalid verification response [SPSM-138]');
            return false;
        } catch (\Exception $e) {
            dd($e->getMessage());
            Session::flash('error', 'Something went wrong. Error:'.$e->getMessage().'[SPSM-138]');
...
//            'return_url'=>'http://oss-framework.eserve.org.bd/spg/callback'
            ...
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: text/xml;charset=utf-8", 'SOAPAction: "http://tempuri.org/DailyStTransaction"'));
            ...
            DB::rollback();
            dd($e->getMessage() . $e->getFile() . $e->getLine().'[SPSC-110]');
        }
    }
 ...
 */
    public function offlinePaymentVerifybackup($paymentId)
    {
        try {
...
//            'return_url'=>'http://oss-framework.eserve.org.bd/spg/callback'
            ...
            $paymentInfo = SonaliPaymentStackHolders::find($decodedPaymentId);
            ...
            $body = '<?xml version="1.0" encoding="utf-8"?>
                ...
                xmlns="http://tempuri.org/">
                <OwnerCode>' . $ownerCode . '</OwnerCode>
                <TransactionNo>' . $paymentInfo->request_id . '</TransactionNo>
                <TransactionDate>' . date('Y-m-d', strtotime($paymentInfo->ref_tran_date_time)) . '</TransactionDate>
                <isEncPwd>true</isEncPwd>
                </GetUpdateAfterCashReceived>
                </soap:Body>
                </soap:Envelope>';
            ...
            $payVerificationReqInfo['password'] = $password;
            $payVerificationReqInfo['OwnerCode'] = $ownerCode;
            $payVerificationReqInfo['TransactionNo'] = $paymentInfo->transaction_id;
            $payVerificationReqInfo['TransactionDate'] = date('Y-m-d', strtotime($paymentInfo->ref_tran_date_time));
            $payVerificationReqInfo['isEncPwd'] = 'true';
            ...
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: text/xml;charset=utf-8", 'SOAPAction: "http://tempuri.org/GetUpdateAfterCashReceived"'));
            ...
            {
                Session::flash('error', 'Your payment is not received by Sonali Bank till now. [SPSC-139]');
                return false;
            }
...
//
            ...
            if (!empty($responseData->TransactionNo) && $responseData->StatusCode == "200") {

                $paymentInfo->is_verified = 1;
                $isUpdateStatus = $paymentInfo->save();
                if ($isUpdateStatus) {
                    Session::flash('success', 'Payment confirmed successfully. [SPSC-117]');
                    return true;
                }
                Session::flash('error', 'Unknown error during payment information update. [SPSC-118]');
                return false;
            }

            $paymentInfo->save();

            Session::flash('error', 'Invalid verification response. [SPSC-119]');
            return false;
        } catch (\Exception $e) {
            Session::flash('error', 'Something went wrong! Error: . ' . $e->getMessage(). '[SPSC-120]');
            return false;
        }
    }
...
//            'return_url'=>'http://oss-framework.eserve.org.bd/spg/callback'
            ...
            if (empty($paymentInfo)) {
                Session::flash('error', 'Invalid payment info. [SPSC-115]');
                return false;
            }
    ...
    public function verifyByRefNoForCounter($id)
    {
        try {
            ...
            $paymentInfo = SonaliPaymentStackHolders::find($id);
            if (empty($paymentInfo)) {
                Session::flash('error', 'Invalid payment info [SPS-159]');
                return false;
            }
   ...
   */
    public function counterPaymentCheck($paymentId = null, $status = null)
    {
        ...
        try {
            ...
            $decodedPaymentId = Encryption::decodeId($paymentId);
            if ($paymentId != null) {
                $sonaliPayment = SonaliPaymentStackHolders::find($decodedPaymentId);
            ...
            DB::commit();
            Session::flash('error', 'Invalid payment id [SPSC-111]');
            return redirect()->back();
        } catch (\Exception $e) {
            DB::rollback();
            Session::flash('error', 'Something went wrong! Error:'.$e->getMessage().' [SPSC-121]');
            return redirect()->back();
        }
    }
 ...
 */
    ...
    {
        if (empty($paymentId)) {
            Session::flash('error', 'Invalid payment id [SPSC-113]');
            return redirect()->back();
        }
        ...
        try {
            ...
            $decodedPaymentId = Encryption::decodeId($paymentId);
            $paymentInfo= SonaliPaymentStackHolders::where('id', $decodedPaymentId)->first();
            ...
            $contents = view("SonaliPaymentStackHolder::counterVoucher-pdf",
            ...
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . ' [SPSC-114]');
    ...
    public function verifyTransactionByRefNo($id)
    {
        ...
        try {
            ...
            DB::rollback();
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . "[SPSC-156]");
            return redirect()->back();
        }
    }
    ...
    public function paymentVoucher($paymentId = null)
    {
        ...
        try {
            ...
            $contents = view("SonaliPaymentStackHolder::paymentVoucher-pdf",
                compact('decodedPaymentId', 'paymentInfo', 'barcode_url', 'companyName', 'emp_info'))->render();
            ...
            Session::flash('error', CommonFunction::showErrorPublic($e->getMessage()) . ' [SPSC-117]');
            return Redirect::back()->withInput();
        }
    }
    ...
    private function getEmployeeInfo($processTypeId, $appId)
    {
        ...
        switch ($processTypeId) {
            case 127: // E-TIN Foreigner
                $appdata = \App\Modules\ETINforeigner\Models\ETINforeigner::where('id', $appId)
                    ->select('appdata')
                    ->first();
                if ($appdata) {
                    $fieldsToExtract = ['taxpayer_name', 'passport_number'];
                    $extractedData = $this->extractFieldsFromJson($appdata->appdata, $fieldsToExtract);
                    $emp_info = (object)[
                        "emp_name" => isset($extractedData['taxpayer_name']) ? $extractedData['taxpayer_name'] : "",
                        "emp_passport_no" => isset($extractedData['passport_number']) ? $extractedData['passport_number'] : ""
                    ];
                }
                break;
            default:
                // Already set to default structure
                break;
        }
        return $emp_info;
    }

    private function extractFieldsFromJson($jsonString, $fields)
    {
        $result = [];
        $data = json_decode($jsonString, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            foreach ($fields as $field) {
                $result[$field] = ''; // Default values for all fields
            }
            return $result;
        }
        foreach ($fields as $field) {
