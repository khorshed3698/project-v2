...
namespace App\Modules\WaiverCondition7\Controllers;
...
class WaiverCondition7Controller extends Controller
{
    private static $processTypeId = 19;
    private static $aclName = "WaiverCondition7";
    ...
    public static function searchWaiverInfo($request, $company_id)
    {
        ...
        if ($getOPNEApprovedRefId->process_type_id == 7 && empty($getOPNEinfo->opn_tracking_no)) {
            $opeData = OfficePermissionExtension::where('id', $getOPNEApprovedRefId->ref_id)->first();
            if (empty($opeData->ref_app_tracking_no)) {
                if (!$opeData) {
                    Session::flash('error', 'Sorry! office permission reference number not found by tracking no!  [WAIVER-1082].' . '<br/>' . Session::get('error'));
                    return redirect()->back();
                }
                Session::put('waiver', $opeData->toArray());
                Session::put('waiver.ref_app_approve_date', $opeData->approved_date ?: null);
            }
        } elseif (in_array($getOPNEApprovedRefId->process_type_id, [6, 7]) && !empty($getOPNEinfo->opn_tracking_no)) {       
            Session::put('waiver', $getOPNEinfo->toArray());
            Session::put('waiver.ref_app_approve_date', $getOPNEinfo->ref_app_approve_date ?: $getOPNEinfo->approved_date);
        }
        
        // Session::put('waiver', $getOPNEinfo->toArray());
        Session::put('waiver.ref_app_tracking_no', $request->get('ref_app_tracking_no'));
        // Session::put('waiver.ref_app_approve_date', $getOPNEinfo->ref_app_approve_date);
        // Session::put('waiver.ref_app_approve_date', $getOPNEinfo->ref_app_approve_date ?: $getOPNEinfo->approved_date);
