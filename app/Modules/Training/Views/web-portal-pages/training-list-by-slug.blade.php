...
{{ !empty($courseDetails->course_title)?$courseDetails->course_title.','.$office_list:$office_list }}
@endsection
@section('meta_title')
...
@section('header-resources')
    @include('public_home.web-page-sections.social-icon-top', ['title' => !empty($courseDetails->course_title)?$courseDetails->course_title:'', 'description' => !empty($courseDetails->course_title)?$courseDetails->course_title:'', 'image' => !empty($courseSliders[0])?asset('/uploads/training/'.$courseSliders[0]):''])
    <style>
        .home-wrapper {
            margin-top: 0;
                                        ...
                                        @if($key == 0)
                                            <div class="item active">
                                                <img class="course_slider_image"
                                                     src="{{ asset('/uploads/training/'.$value) }}" alt="{{ $courseDetails->course_title }}"
                                                     onerror="this.src=`{{asset('/assets/images/no-image.png')}}`" title="{{ $courseDetails->course_title }}"/>
                                            </div>
                                        @else
                                            <div class="item">
                                                <img class="course_slider_image"
                                ...
                                <div class="carousel-inner">
                                    <div class="item active">
                                        <img class="course_slider_image" src="{{asset('/assets/images/no-image.png')}}"
                                             alt="{{ $courseDetails->course_title }}" title="{{ $courseDetails->course_title }}"/>
                                    </div>
                                </div><!--/.carousel-inner-->
                            @endif
                        </div><!--/.courseCarousel-->
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="other_location_program_title">
                        @if($totalScheduleCourse > 0)
                            <h2>{{ $courseDetails->course_title }} যে সকল স্থানে চলমান আছে</h2>
                        @else
                            <div class="course_title_sec">
                                <div class="row">
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.responseCode == 1) {
                        ...
                        $('#loadUpcomingTrainingByCourseDiv').html('');
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    //$('#loadUpcomingTrainingByCourseDiv').html('<h3 class="text-center text-info">'+errorThrown+'</h3>');
                    $('#loadUpcomingTrainingByCourseDiv').html('');
                },
                complete: function () {
