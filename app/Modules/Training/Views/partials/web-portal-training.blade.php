...
@section('header-resources')
    ...
    <style>
        ...
        .training_search_keywords{
            padding: 15px 0 0 0 ;
        }
        .training_search_keywords .training_search_keywords_text{
            margin: 0;
            font-weight: bold;
        }
    </style>
...
{{--                    </li>--}}
    ...
    <script>
        ...
        $('.dynamic_tab').on('click', function () {
            $('#training_search_keywords_row').hide();
            // var flag = $(this).attr('aria-controls');
            flag = $(this).attr('aria-controls');
            $.ajax({
                type: "GET",
        ...
        $(".filter").on('click', function () {
            ...
            txtSearch = '';
            var training_search_keywords_text = 'আপনি সার্চ করেছেন :';
            if (dataId == 'input-search') {
                ...
                training_search_keywords_text +=  ' '+txtSearch;
            } else {
                if (course_id == '' && district_id == '' && office_id == '') {
                    toastr.options = {
                        positionClass: "toast-bottom-right",
                    }
                    toastr.error('<b>Please select at list one. (District/Office/Course)</b>');
                    return false;
                }
                if(division_id){
                    training_search_keywords_text +=  ' বিভাগ='+$("#division option:selected").text();
                }
                if(district_id){
                    training_search_keywords_text +=  ' জেলা='+$("#district option:selected").text();
                }
                if(office_id){
                    training_search_keywords_text +=  ' অফিস='+$("#office option:selected").text();
                }
                if(course_id){
                    training_search_keywords_text +=  ' কোর্স='+$("#course option:selected").text();
                }
            }
            $.ajax({
                type: "GET",
                ...
                success: function (response) {
                    if (response.responseCode == 1) {
                        $('.dropdown').fadeOut();
                        $('#load_content').html(response.html);
                        $('#training_search_keywords_row').show();
                        $('.training_search_keywords_text').text(training_search_keywords_text);
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    $('#load_content').html('<div class="row"><div class="col-md-12"><h3 class="text-center text-danger">Something went wrong !</h3></div></div>');
