{!! Form::open(array('url' => '/settings/api-stakeholder-distribution-update/','method' => 'post', 'class' => 'form-horizontal smart-form','id'=>'stakeholderDistribution',
        'enctype' =>'multipart/form-data', 'files' => 'true', 'role' => 'form')) !!}

<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span><span class="sr-only">Close</span>
    </button>
    <h4 class="modal-title" id="myModalLabel"><i class="fa fa-th-large"></i> Edit Stakeholder Payment Distribution</h4>
</div>
...
<div class="modal-body">
    ...
    <input name="data_id" type="hidden"
           value="{{ \App\Libraries\Encryption::encodeId($data->id) }}">
    <input name="process_type_id" type="hidden"
           value="{{ \App\Libraries\Encryption::encodeId($data->process_type_id) }}">
    <input name="stackholder_id" type="hidden"
           value="{{ \App\Libraries\Encryption::encodeId($data->stackholder_id) }}">

    <div class="row">
        <div class="col-lg-12">
            <div class="form-group col-md-12 {{$errors->has('receiver_account_no') ? 'has-error' : ''}}">
                {!! Form::label('receiver_account_no','Receiver Account No : ',['class'=>'col-md-4 text-right required-star']) !!}
                <div class="col-md-8">
                    {!! Form::text('receiver_account_no', $data->receiver_account_no, ['class'=>'form-control required input-sm','placeholder'=>'Receiver Account No']) !!}
                    {!! $errors->first('receiver_account_no','<span class="help-block">:message</span>') !!}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-12" id="pay_amount">
            <div class="form-group col-md-12 {{$errors->has('amount') ? 'has-error' : ''}}">
                ...
                <div class="col-md-8">
                    {!! Form::text('pay_amount', $data->amount, ['class'=>'form-control required input-sm', 'placeholder'=>'Amount']) !!}
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="form-group col-md-12 {{$errors->has('category') ? 'has-error' : ''}}">
                {!! Form::label('category','Category: ',['class'=>'col-md-4 text-right required-star']) !!}
                <div class="col-md-8">
                    {!! Form::text('category', $data->category, ['class'=>'form-control required input-sm', 'placeholder'=>'Category']) !!}
        ...
        <div class="col-md-12">
            <div class="form-group col-md-12 {{$errors->has('status') ? 'has-error' : ''}}">
                ...
                <div class="col-md-8 {{$errors->has('status') ? 'has-error' : ''}}">
                    <label>{!! Form::radio('status', 1,  ($data->is_active == 1) ? true:false, ['class'=>'required', 'id' => 'yes']) !!}
                        Active</label>
                    <label>{!! Form::radio('status', 0, ($data->is_active == 0) ? true:false, ['class'=>' required', 'id' => 'no']) !!}
                        Inactive</label>
...
<div class="modal-footer" style="text-align:left;">
    ...
    <div class="pull-right">
        @if(ACL::getAccsessRight('settings','A'))
            <button type="submit" class="btn btn-info" id="action_btn" name="actionBtn" value="draft">
                <i class="fa fa-chevron-circle-right"></i> Edit
            </button>
        @endif
    </div>
