...
<script language="javascript">
    ...
    jQuery(function () {
        jQuery('#submitFromPreviewBtn').click(function (e) {
            window.opener.document.getElementById("WorkPermitCancelationForm").setAttribute("target", "_self");
            window.opener.jQuery("#WorkPermitCancelationForm").submit();
            window.close();
        });
    });
...
//        window.opener.jQuery('#' + id + ' option:[value="' + val + '"]').replaceWith("<option value='" + val + "' selected>" + text + "</option>");
    ...
    function CloseMe()
    {
        ...
        window.opener.jQuery("#WorkPermitCancelationForm-p-4").css({"display": "block"});
