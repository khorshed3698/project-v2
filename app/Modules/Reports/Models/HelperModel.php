...
//    return '';
    ...
    public function createHTMLTable($arr,$max_length = 25) {
        $arr = json_decode(json_encode($arr), true);
        $table = '<table aria-label="Detailed Report Data Table" class="table basicDataTable dataTable" id="dTable_'.time().'"><thead>';
        if(isset($arr[0])) {
    ...
    public function formatTDValue($cell,$maxwd=25){
        if($cell) {
    ...
    public function report_gen($id, $data, $report_title, $link = '', $heading = '') {
        $dataTablePara = '';
        $showaction = false;
        $hiddenFields = array();
        $cols = array();
        ...
        <div class="hero-widget well well-sm">
            ...
            <?php if (count($data) > 0) { ?>
                ...
                <table aria-label="Detailed Report Data Table" id="dbt_<?php echo $id; ?>" class="datatable table-rpt-border table table-responsive table-condensed" id="tblQInfo">
                    <thead>
                    <tr class="d-none">
...
//                            } $count++;
    ...
    public function getWebLink($website){
    ...
    public function mobileMask($mobile){
        if(strlen($mobile)>13) {
