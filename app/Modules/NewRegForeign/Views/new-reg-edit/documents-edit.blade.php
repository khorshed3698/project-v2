        ...
        <div class="panel-body">
            <div class="row">
                <div class="col-md-12">
                        ...
                        @foreach($Rjsc_NrForms as $info)
                            @if($info->is_extra == 0)
                                ...
                                <tr>
                                    ...
                                    <td style="">
                                        @if($info->doc_file_path !='' && $info->doc_file_path !=null)
                                        <a target="_blank" rel="noopener" class="btn btn-xs btn-primary"
                                           href="{{URL::to('/uploads/'.$info->doc_file_path)}}"
                                           title="{{ $info->trade_license }}">
                                    ...
                                    <td>
                                        @if(\App\Libraries\CommonFunction::checkForeignFileSubmission($applicationId))
                                            @if(\App\Libraries\CommonFunction::getForeignFileUploadedStatus($info->doc_info_id, $applicationId))
                                                <center>
                                                    <b> Uploaded</b>
                                                </center>
                                            @endif
                        ...
                        @foreach($extraFiles as $info)
                            @if($info->is_extra == 1)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(\App\Libraries\CommonFunction::checkForeignFileSubmission($applicationId))
                                            @if(\App\Libraries\CommonFunction::getForeignFileUploadedStatus($info->id, $applicationId))
                                                <center>
                                                    <b> Uploaded</b>
                                                </center>
                                            @endif
                    ...
                    {!! Form::open(array('url' => '/new-reg-foreign/new-reg-page/final-submit','method' => 'post', 'id' => 'd','role'=>'form')) !!}
                    {{--  need to add after --}}
                ...
                @include('NewRegForeign::preview.doc-tab')
...
<script type="text/javascript">
    ...
    function uploadFilesUpload(base64_img, input, target) {
        ...
        $.ajax({
            url: '{{url('new-reg-foreign/new-store/upload')}}',
            type: 'post',
            data: {
                _token: _token,
                app_id: app_id,
