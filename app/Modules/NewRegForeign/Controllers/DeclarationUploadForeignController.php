...
class DeclarationUploadForeignController extends Controller
{
...
//        $this->validate($request, $rules,$messages);
        ...
        try {
            ...
            $Rjsc_NrForms = ProcessList::leftjoin('app_documents',
                ...
                ->get(['app_documents.*']);
            foreach ($Rjsc_NrForms as $file){
                $rjscFile = RjscNrfSubmitForms::firstOrNew([
                    'ref_id' => $file->doc_info_id,
                    'app_id' => $app_id
                ]);
//                if ($file->doc_file_path == '' || $file->doc_file_path == null){
//                    DB::rollback();
//                    Session::flash('error', 'You must have attach necessary document in office permission New.');
...
//                }
                ...
                $pdfFile_url = public_path()."/uploads/" . $file->doc_file_path;
//                dd($pdfFile_url);
                $pdfFile = file_get_contents($pdfFile_url);
                $rjscFile->ref_id = $file->doc_info_id;
                $rjscFile->app_id = $app_id;
                $rjscFile->form_name = $file->doc_name;
                $rjscFile->doc ='data:application/pdf;base64,'.base64_encode($pdfFile);
                $rjscFile->file = $file->doc_file_path;
                $rjscFile->is_extra = 0;
                $rjscFile->status = 1;
                $rjscFile->save();
            }
...
//                dd($appData);
            ...
            $appData->declaration_position_id=$request->get('declaration_position_id');
            ...
            $appData->declaration_district_id=$request->get('declaration_district_id');
            /*$appData->upload_doc_name_id=$request->get('upload_doc_name_id');

            $fileZip = $request->file('upload_scaned_copy');

            if(isset($fileZip) && !empty($fileZip)){
//                dd('success');
                $fileMime = $fileZip->getClientMimeType();

                $validMimes = array('application/zip','application/x-rar','application/x-rar-compressed','application/octet-stream','application/x-zip-compressed','multipart/x-zip');
                $fileExt = array_search($fileMime, $validMimes, true);
                //dd($fileMime,$fileExt);
                if($fileExt > -1){
                    $size = $fileZip->getClientSize();
                    $filesize = ($size / 1000);
                    if($filesize < 2000){
                        $currentDate = Carbon::now()->toDateString();
                        $filename = $currentDate.'-'.uniqid().'.'.
                            $fileZip->getClientOriginalExtension();
                        if(!file_exists('upload_scaned_copy')){
                            mkdir('upload_scaned_copy',0777,true);
                        }
                        $fileZip->move('upload_scaned_copy',$filename);
                        $appData->upload_scaned_copy=$filename;
                    }else{
                        Session::flash('error','Upload copy file must lower than 2 kb');
                        return Redirect::to(URL::previous() . "#step5");
                    }
                }else{
                    Session::flash('error','Upload copy must be zip or rar file');
                    return Redirect::to(URL::previous() . "#step5");
                }
            }*/

//            $image = $request->file('upload_scaned_copy');
//            if(isset($image) && !empty($image)){
//                $currentDate = Carbon::now()->toDateString();
//                $imagename = $currentDate.'-'.uniqid().'.'.
//                    $image->getClientOriginalExtension();
//                if(!file_exists('upload_scaned_copy')){
//                    mkdir('upload_scaned_copy',0777,true);
//                }
//                $image->move('upload_scaned_copy',$imagename);
//
//                $appData->upload_scaned_copy=$imagename;
//            }
            ...
            /*if(isset($fileZip) && !empty($fileZip)){
                $nrDoc = new RjscNrDoc();
                $nrDoc->ref_id = $appData->id;
                $nrDoc->doc_name = $filename;
                $nrDoc->doc = $request->get('scan_copy_base_64');
                $nrDoc->status = -10;
                $nrDoc->save();
            }*/
            ...
            return redirect('licence-applications/company-registration-foreign/add#step12');
    ...
    public function asciicheck(){
       ...
       /* echo '<pre>';
        print_r($string);
        //print_r($string['req_json_data']['registrationData']['listOfDirectors']);
       ...
       /* foreach($string['req_json_data']['registrationData']['moaObjectiveList'] as $key => $value){
            echo 'moaObjectiveList'.$key.' <br/><br/>';
            foreach ($value as $key1=>$value2){
                echo $key1.' :'.$value2;
                if(mb_detect_encoding($value2, 'ASCII', true)){
                    echo '    : ASCI VERIFIED'.' <br/>';
                }else{
