...
class IrmsApiController extends Controller
{
    ...
    public function callBack(Request $request)
    {
        ...
        if (
            empty($request->get('client_id')) || empty($request->get('client_secret_key')) ||
    ...
    public function companyList(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                $responseEncode = $this->IrmsDataResponse($this->message, 'BIDA-COMPANY-LIST', $this->status_code, '', '', $requestData['request_at']);
            ...
            if (!empty($token)) {
                ...
                // store request
                $api_response = new ModelsApiResponse();
                $api_response->api_id = 6; // List Data
                $api_response->external_request_id = $requestData['request_id'];
                $api_response->endpoint = $request->fullUrl();
                $api_response->method = $request->method();
                $api_response->headers = '';
                $api_response->body = json_encode($request->all());
                $api_response->created_at = $requestData['request_at'];
                $api_response->save();
                    ...
                    if (date('Y-m-d H:i:s') < $clientData->oauth_token_expire_at) {
                        ...
                        if ($tokenService->checkTokenValidity($token, $clientData->encryption_key)) {
                            ...
                            if ($isAuthorizedClientUrl['return_type'] === true) {
                                ...
                                $companyData = CompanyInfo::query()
                                    ->select('company_name', 'company_name_bn', DB::raw('IF(business_category = 1, "private", "government") as business_category'))
                                    ->where('is_approved', 1)
                                    ->where('company_status', 1)
                                    ->where('is_rejected', 'no')
                                    ->where('sent_to_irms', 0)
                                    ->limit(5);

                                if ($companyData->get()->count() == 0) {
                                    $this->message = 'No company information found';
                                    $this->status_code = HTTPResponse::HTTP_NO_CONTENT;
                                } else {
                                    $this->response_data = $companyData->get();
                                    $this->status_code = HTTPResponse::HTTP_OK;
                                    $this->message = 'Company Information';
                                    $companyData->update(['sent_to_irms' => 1]);
                                }
                            } else {
                                $this->message = 'Request could not be processed due to unauthorized ip/url: ' . $isAuthorizedClientUrl['return_data'];
                                $this->status_code = HTTPResponse::HTTP_UNAUTHORIZED;
            ...
            $responseEncode = $this->IrmsDataResponse($this->message, 'BIDA-COMPANY-LIST', $this->status_code, $this->response_data, isset($api_response->response_id) ? $api_response->response_id : '', $requestData['request_at']);

            return response()->json($responseEncode);
        } catch (\Exception $e) {
            Log::error('companyList: ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRMS-1011]');
            return response()->json($this->IrmsDataResponse('Sorry ! An internal error has occurred', 'BIDA-COMPANY-LIST', HTTPResponse::HTTP_INTERNAL_SERVER_ERROR, $this->response_data, '', ''));
        }
    }
    ...
    public function companyStore(Request $request)
    {
        try {
            ...
            $requestData = [
                ...
                'request_id' => trim($request->get('request_id')),
                'request_at' => date("Y-m-d H:i:s"),
                'company_name_en' => trim($request->get('company_name_en')),
            ...
            $validator = Validator::make($requestData, [
                ...
                'business_category' => 'required|in:1,2',
            ]);
            ...
            if ($validator->fails()) {
                ...
                $responseEncode = $this->IrmsDataResponse($this->message, 'BIDA-COMPANY-STORE', $this->status_code, '', '', $requestData['request_at']);
            ...
            if (!empty($token)) {
                ...
                // store request
                $api_response = new ModelsApiResponse();
                $api_response->api_id = 7; // Store Data
                $api_response->external_request_id = $requestData['request_id'];
                $api_response->endpoint = $request->fullUrl();
                $api_response->method = $request->method();
                $api_response->headers = '';
                    ...
                    if (date('Y-m-d H:i:s') < $clientData->oauth_token_expire_at) {
                        ...
                        if ($tokenService->checkTokenValidity($token, $clientData->encryption_key)) {
                            ...
                            if ($isAuthorizedClientUrl['return_type'] === true) {
                                ...
                                $companyData = CompanyInfo::where('company_name', $requestData['company_name_en'])
                                    ->orWhere('company_name_bn', $requestData['company_name_bn'])
                                    ->exists();

                                if ($companyData) {
                                    $this->message = 'Company Information already exists';
                                    $this->status_code = HTTPResponse::HTTP_ALREADY_REPORTED;
                                } else {
                                    $companyData = CompanyInfo::create([
                                        'company_name' => $requestData['company_name_en'],
                                        'company_name_bn' => $requestData['company_name_bn'],
                                        'business_category' => $requestData['business_category'],
                                        'recieved_from_irms' => 1,
                                        'is_approved' => 1,
                                        'company_status' => 1,
                                    ]);

                                    $this->message = 'Company Information saved successfully';
                                    $this->status_code = HTTPResponse::HTTP_OK;
                                }
                            } else {
                                $this->message = 'Request could not be processed due to unauthorized ip/url: ' . $isAuthorizedClientUrl['return_data'];
            ...
            $responseEncode = $this->IrmsDataResponse($this->message, 'BIDA-COMPANY-STORE', $this->status_code, $this->response_data, isset($api_response->response_id) ? $api_response->response_id : '', $requestData['request_at']);

            return response()->json($responseEncode);
        } catch (\Exception $e) {
            Log::error('companyList: ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRMS-1011]');
            return response()->json($this->IrmsDataResponse('Sorry ! An internal error has occurred', 'BIDA-COMPANY-STORE', HTTPResponse::HTTP_INTERNAL_SERVER_ERROR, $this->response_data, '', ''));
        }
    }
    ...
    public function companyUpdate(Request $request)
    {
        try {
            ...
            $requestData = [
                ...
                'request_id' => trim($request->get('request_id')),
                'request_at' => date("Y-m-d H:i:s"),
                'ex_company_name_en' => trim($request->get('ex_company_name_en')),
                'ex_company_name_bn' => trim($request->get('ex_company_name_bn')),
                'ex_business_category' => $request->get('ex_business_category') ? trim($request->get('ex_business_category')) : null,
                'pro_company_name_en' => trim($request->get('pro_company_name_en')),
                'pro_company_name_bn' => trim($request->get('pro_company_name_bn')),
                'pro_business_category' => $request->get('pro_business_category') ? trim($request->get('pro_business_category')) : null,
            ];
            ...
            $validator = Validator::make($requestData, [
                ...
                'client_secret_key' => 'required',
                'request_id' => 'required',
                'ex_company_name_en' => 'required',
                'ex_company_name_bn' => 'required',
                'ex_business_category' => 'in:1,2',
                'pro_company_name_en' => 'required',
                'pro_company_name_bn' => 'required',
                'pro_business_category' => 'in:1,2',
            ]);
            ...
            if ($validator->fails()) {
                ...
                $responseEncode = $this->IrmsDataResponse($this->message, 'BIDA-COMPANY-UPDATE', $this->status_code, '', '', $requestData['request_at']);
            ...
            if (!empty($token)) {
                ...
                // store request
                $api_response = new ModelsApiResponse();
                $api_response->api_id = 8; // Update Data
                $api_response->external_request_id = $requestData['request_id'];
                $api_response->endpoint = $request->fullUrl();
                $api_response->method = $request->method();
                $api_response->headers = '';
                    ...
                    if (date('Y-m-d H:i:s') < $clientData->oauth_token_expire_at) {
                        ...
                        if ($tokenService->checkTokenValidity($token, $clientData->encryption_key)) {
                            ...
                            if ($isAuthorizedClientUrl['return_type'] === true) {
                                ...
                                $companyData = CompanyInfo::leftJoin('company_association_request', 'company_info.id', '=', 'company_association_request.requested_company_id')
                                    ->where('company_info.company_name', $requestData['ex_company_name_en'])
                                    ->orWhere('company_info.company_name_bn', $requestData['ex_company_name_bn'])
                                    ->where('company_info.recieved_from_irms', 1)
                                    ->where('company_info.is_eligible', 0)->first(['company_info.*', 'company_association_request.requested_company_id']);

                                if ($companyData && $companyData->requested_company_id == null) {

                                    $updatedData = CompanyInfo::find($companyData->id);

                                    $updatedData->company_name = $requestData['pro_company_name_en'];
                                    $updatedData->company_name_bn = $requestData['pro_company_name_bn'];
                                    if ($requestData['pro_business_category']) {
                                        $updatedData->business_category = $requestData['pro_business_category'];
                                    }
                                    ...
                                    $this->status_code = HTTPResponse::HTTP_OK;
                                } else if ($companyData && $companyData->requested_company_id != null) {
                                    $this->message = 'Company Information Already Associated';
                                    $this->status_code = HTTPResponse::HTTP_ALREADY_REPORTED;
                                } else {
                                    $this->message = 'Company Information not found';
                                    $this->status_code = HTTPResponse::HTTP_NOT_FOUND;
                                }
                            } else {
                                $this->message = 'Request could not be processed due to unauthorized ip/url: ' . $isAuthorizedClientUrl['return_data'];
            ...
            $responseEncode = $this->IrmsDataResponse($this->message, 'BIDA-COMPANY-UPDATE', $this->status_code, $this->response_data, isset($api_response->response_id) ? $api_response->response_id : '', $requestData['request_at']);

            return response()->json($responseEncode);
        } catch (\Exception $e) {
            Log::error('companyUpdate: ' . $e->getMessage() . ' ' . $e->getFile() . ' ' . $e->getLine() . ' [IRMS-1280]');
            return response()->json($this->IrmsDataResponse('Sorry ! An internal error has occurred', 'BIDA-COMPANY-UPDATE', HTTPResponse::HTTP_INTERNAL_SERVER_ERROR, $this->response_data, '', ''));
        }
    }
}
