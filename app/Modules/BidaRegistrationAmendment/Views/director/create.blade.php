    ...
    <div class="row" id="nid_save" hidden>
        <div class="col-md-offset-2 col-md-8">
            <fieldset class="scheduler-border">
                ...
                {{-- Amendment type	 --}}
                <div class="form-group has-feedback amendment_type_div {{ $errors->has('amendment_type') ? 'has-error' : ''}}">
...
<script>
    ...
    function VerifyNID() {
        ...
        var bra_approval_manually = $('input[name="is_bra_approval_manually"]:checked').val() || '';
        if (bra_approval_manually == 'no') {
            $('.amendment_type_div').hide();
            $('#nid_save input[name="amendment_type"][value="proposed"]').prop('checked', true);
        }
        else {
            $('.amendment_type_div').show();
            $('#nid_save input[name="amendment_type"]').prop('checked', false);
        }
    ...
    function VerifyETIN() {
        ...
        if (bra_approval_manually == 'no') {
            $('.amendment_type_div').hide();
            $('#tin_save input[name="amendment_type"][value="proposed"]').prop('checked', true);
            
        }
        else {
            $('.amendment_type_div').show();
            $('#tin_save input[name="amendment_type"]').prop('checked', false);
        }
    ...
    $(document).ready(function(){
        ...
        $('#passport_verify').on('click', function() {
            ...
            var bra_approval_manually = $('input[name="is_bra_approval_manually"]:checked').val() ? $('input[name="is_bra_approval_manually"]:checked').val() : '';
            if (bra_approval_manually == 'no') {
                $('.amendment_type_div').hide();
                $('#passport_div input[name="amendment_type"][value="proposed"]').prop('checked', true);
            }
            else {
                $('.amendment_type_div').show();
                $('#passport_div input[name="amendment_type"]').prop('checked', false);
            }
