        ...
        if ($process_list_data1->num_rows > 0) {
            ...
            while($row6 = $process_list_data1->fetch_assoc()) {
                ...
                if($exit_holiday == $max_processing_time){
                    $sql10 = "select id from users 
                              where desk_id REGEXP '^([0-9]*[,]+)*$desk_to([,]+[,0-9]*)*$' limit 1";
...
//                6. Set current_desk = next desk, current_status = next_status, Remarks = "Automatically processed", Updated_by = "***".
                    $sql8 = "UPDATE process_list SET desk_id='$desk_to', user_id='$userId',  status_id='$status_to', process_desc='Automatically processed', updated_by='1',on_behalf_of_user='1'  WHERE id=$process_id";
                    $conn->query($sql8);
    ...
    echo 'Auto process could not set to process path ';
}
$result->close();
