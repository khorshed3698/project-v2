...
if ($result1 = $mysqli->query("SELECT * FROM pdf_print_requests_queue WHERE job_receiving_status=0  ORDER BY id DESC LIMIT 5")) {
    while ($row = $result1->fetch_assoc()) {
        ...
        $data['data'] = array(
            ...
            'param' => array( // param must be same as request entry data from each f
                //'app_id' => (int)$app_id, // app_id
                'app_id' => (int)$app_id, // app_id
                'process_type_id' => (int)$process_type_id, // Process type id of each module
            )
        );
        ...
        if (!empty($dataResponse->response) && $dataResponse->response->status == 1) {
            ...
            switch ($process_type_id) {
                case 1:
                    // Visa Recommendation Certificate Update
                    $appUpdateSQL = "UPDATE vr_apps SET certificate_link='" . $attachmentUrl . "' WHERE id=$app_id";
                    break;
                default:
            }

            $appUpdateSQLStatus = $mysqli->query($appUpdateSQL);
        }
//        else {
...
//        }
        ...
        if (!empty($dataResponse->response) && ($dataResponse->response->status == -1 || $dataResponse->response->status == 1) && $appUpdateSQLStatus) {
            $status = 1; // data sent
