    ...
    <script>
        async function fetchInsightDbNotice() {
            try {
                ...
                return makeInsightDbNoticeApiCall(token);
            } catch (error) {
        ...
        async function makeInsightDbNoticeApiCall(token) {
            const base_url = "{{ config('app.insightdb_api_base_url') }}";
            const endpoint = "{{ config('app.landing_page_notice') }}";
            ...
            try {
                ...
                if (response.status === 401) {
                    const newToken = await TokenManager.fetchToken("/bida-oss-landing/insightdb-token");
                    return makeInsightDbNoticeApiCall(newToken);
                }
